#!/usr/bin/env python3
"""Apply review-notes patch CSV to an ingredient workbook copy."""
...
try:
    ...
    raise SystemExit(
        "openpyxl is required to apply workbook review-notes patches. Install it in the local Python environment first."
    ) from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply ingredient review-notes patch CSV to a workbook copy.")
    parser.add_argument("--ingredient-xlsx", required=True, help="Source ingredient workbook")
    parser.add_argument("--patch-csv", required=True, help="Apply-ready review-notes patch CSV")
    ...
    for patch in patch_rows:
        ...
        review_notes_cell = sheet.cell(row=row_number, column=header_index["review_notes"])
        current_review_notes = normalize_text(review_notes_cell.value)
        expected_review_notes = normalize_text(patch.get("existing_review_notes"))

        if current_review_notes != expected_review_notes:
            skipped_conflicts.append(
                {
                    "record_id": record_id,
                    "current_review_notes": current_review_notes,
                    "expected_review_notes": expected_review_notes,
                }
            )
            continue

        review_notes_cell.value = normalize_text(patch.get("patch_review_notes"))
