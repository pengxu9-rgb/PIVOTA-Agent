...
{
  echo "query,resolved,reason,confidence,latency_ms,matched_product_id,matched_merchant_id,top_candidate_id,top_candidate_title,sources"
} > "$report_csv"

{
  ...
  echo
  echo "| query | resolved | reason | confidence | latency_ms | matched_product_id | matched_merchant_id | top_candidate | sources |"
  echo "| --- | --- | --- | --- | --- | --- | --- | --- | --- |"
} > "$report_md"

for query in "${QUERIES[@]}"; do
  total=$((total + 1))
  payload="$(python3 - "$query" "$LANG_CODE" "$TIMEOUT_MS" "$UPSTREAM_RETRIES" <<'PY'
import json, sys
q = sys.argv[1]
lang = sys.argv[2]
...
print(
    f"{str(resolved).lower()}\t{nz(reason)}\t{nz(confidence_str)}\t{nz(latency_str)}\t"
    f"{nz(matched_product)}\t{nz(matched_merchant)}\t{nz(top_pid)}\t{nz(top_title)}\t{nz(source_summary)}"
)
' <<< "$resp")"

  IFS=$'\t' read -r resolved reason confidence latency matched_product matched_merchant top_pid top_title source_summary <<< "$parsed"
  ...
  printf '"%s",%s,"%s",%s,%s,"%s","%s","%s","%s","%s"\n' \
    "$query_csv" "$resolved" "$reason" "${confidence:-}" "${latency:-}" \
    "$matched_product" "$matched_merchant" "$top_pid" "$top_title_csv" "$source_csv" >> "$report_csv"

  query_md="${query//|/\\|}"
  top_md="${top_title//|/\\|}"
  sources_md="${source_summary//|/\\|}"
  echo "| ${query_md} | ${resolved} | ${reason:-n/a} | ${confidence:-n/a} | ${latency:-n/a} | ${matched_product:-n/a} | ${matched_merchant:-n/a} | ${top_pid:-n/a} ${top_md:+(${top_md})} | ${sources_md:-n/a} |" >> "$report_md"
done
