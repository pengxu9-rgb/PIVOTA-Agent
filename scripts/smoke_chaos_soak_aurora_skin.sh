...
check_rolling_thresholds() {
  local check_output
  check_output="$(python3 - "$EVENTS_FILE" "$MAX_5XX_RATE_5M" "$MAX_GUARD_FALLBACK_RATE_10M" <<'PY'
import json
import sys
...
print(json.dumps({"stop": False, "reason": "", "rate_5xx_5m": rate_5xx, "rate_guard_10m": rate_guard}))
PY
)"
  local should_stop
...
record_response() {
  ...
  if (( empty_cards > 0 )); then TOTAL_EMPTY_CARDS=$(( TOTAL_EMPTY_CARDS + 1 )); fi
  ...
  jq -cn \
    ...
    '{
      ...
      schema_violation: (if ($schema_ok=="true") then 0 else 1 end),
...
print_stats() {
  ...
  log_line "phase=${CURRENT_LOAD_PHASE} responses=${TOTAL_RESPONSES} 5xx_rate=${r_5xx} timeout_degraded_rate=${r_timeout} guard_fallback_rate=${r_guard} schema_violation_rate=${r_schema}"
}
...
write_summary() {
  ...
  {
    echo "status,stop_reason,total_request_iterations,total_responses,http_5xx,timeout_degraded,reco_output_guard_fallback,schema_violations,empty_cards,notice_without_actions,safety_with_recommendations,low_med_treatment_leak,http_5xx_rate,timeout_degraded_rate,reco_output_guard_fallback_rate,schema_violation_rate"
    echo "${status},${reason},${TOTAL_REQUEST_ITERATIONS},${TOTAL_RESPONSES},${TOTAL_HTTP_5XX},${TOTAL_TIMEOUT_DEGRADED},${TOTAL_GUARD_FALLBACK},${TOTAL_SCHEMA_VIOLATIONS},${TOTAL_EMPTY_CARDS},${TOTAL_NOTICE_WITHOUT_ACTIONS},${TOTAL_SAFETY_WITH_RECO},${TOTAL_LOW_MED_LEAK},${r_5xx},${r_timeout},${r_guard},${r_schema}"
  } >"$SUMMARY_CSV"
}
