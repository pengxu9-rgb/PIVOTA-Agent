...
STAGING_AGENT_API_KEY="${STAGING_AGENT_API_KEY:-}"
...
if [[ -z "${GATEWAY_GOVERNANCE_LOG_INPUT_PATH}" && -z "${GATEWAY_GOVERNANCE_SHADOW_SAMPLE_PATH}" && "${GATEWAY_GOVERNANCE_AUTO_FETCH}" != "1" && "${GATEWAY_GOVERNANCE_AUTO_FETCH}" != "true" ]]; then
...
run_step \
  "readiness_audit" \
  bash -lc "cd '${REPO_ROOT}' && OUT_DIR='${READINESS_ROOT}' GATEWAY_GOVERNANCE_LOG_INPUT_PATH='${GATEWAY_GOVERNANCE_LOG_INPUT_PATH}' GATEWAY_GOVERNANCE_SHADOW_SAMPLE_PATH='${GATEWAY_GOVERNANCE_SHADOW_SAMPLE_PATH}' GATEWAY_GOVERNANCE_AUTO_FETCH='${GATEWAY_GOVERNANCE_AUTO_FETCH}' GATEWAY_GOVERNANCE_LOG_INPUT_AUTOMATED='${GATEWAY_GOVERNANCE_LOG_INPUT_AUTOMATED}' GATEWAY_GOVERNANCE_RAILWAY_PROJECT='${GATEWAY_GOVERNANCE_RAILWAY_PROJECT}' GATEWAY_GOVERNANCE_RAILWAY_ENVIRONMENT='${GATEWAY_GOVERNANCE_RAILWAY_ENVIRONMENT}' GATEWAY_GOVERNANCE_RAILWAY_SERVICE='${GATEWAY_GOVERNANCE_RAILWAY_SERVICE}' GATEWAY_GOVERNANCE_RAILWAY_WORKSPACE='${GATEWAY_GOVERNANCE_RAILWAY_WORKSPACE}' GATEWAY_GOVERNANCE_RAILWAY_DEPLOYMENT='${GATEWAY_GOVERNANCE_RAILWAY_DEPLOYMENT}' GATEWAY_GOVERNANCE_FETCH_LINES='${GATEWAY_GOVERNANCE_FETCH_LINES}' '${npm_bin}' run audit:readiness:commerce-core"
...
run_step \
  ...
  bash -lc "cd '${REPO_ROOT}' && OUT_DIR='${GATEWAY_DAILY_ROOT}' GATEWAY_GOVERNANCE_LOG_INPUT_PATH='${GATEWAY_GOVERNANCE_LOG_INPUT_PATH}' GATEWAY_GOVERNANCE_SHADOW_SAMPLE_PATH='${GATEWAY_GOVERNANCE_SHADOW_SAMPLE_PATH}' GATEWAY_GOVERNANCE_AUTO_FETCH='${GATEWAY_GOVERNANCE_AUTO_FETCH}' GATEWAY_GOVERNANCE_RAILWAY_PROJECT='${GATEWAY_GOVERNANCE_RAILWAY_PROJECT}' GATEWAY_GOVERNANCE_RAILWAY_ENVIRONMENT='${GATEWAY_GOVERNANCE_RAILWAY_ENVIRONMENT}' GATEWAY_GOVERNANCE_RAILWAY_SERVICE='${GATEWAY_GOVERNANCE_RAILWAY_SERVICE}' GATEWAY_GOVERNANCE_RAILWAY_WORKSPACE='${GATEWAY_GOVERNANCE_RAILWAY_WORKSPACE}' GATEWAY_GOVERNANCE_RAILWAY_DEPLOYMENT='${GATEWAY_GOVERNANCE_RAILWAY_DEPLOYMENT}' GATEWAY_GOVERNANCE_FETCH_LINES='${GATEWAY_GOVERNANCE_FETCH_LINES}' '${npm_bin}' run audit:gateway-governance:daily"
...
aurora_manual_review_report_path=""
aurora_manual_review_summary_path=""
if [[ -n "${STAGING_AUTH_TOKEN}" || -n "${STAGING_AGENT_API_KEY}" ]]; then
  run_step \
    "aurora_manual_review" \
    bash -lc "cd '${REPO_ROOT}' && AUTH_TOKEN='${STAGING_AUTH_TOKEN}' AGENT_API_KEY='${STAGING_AGENT_API_KEY}' OUT_DIR='${AURORA_MANUAL_REVIEW_ROOT}' '${node_bin}' scripts/run_celestial_commerce_core_aurora_manual_review.js --base-url '${STAGING_BASE_URL}' --endpoint '/agent/shop/v1/invoke' --out-dir '${AURORA_MANUAL_REVIEW_ROOT}'"
  aurora_manual_review_report_path="$(latest_match "${AURORA_MANUAL_REVIEW_ROOT}" 'README.md')"
  aurora_manual_review_summary_path="$(latest_match "${AURORA_MANUAL_REVIEW_ROOT}" 'summary.json')"
else
