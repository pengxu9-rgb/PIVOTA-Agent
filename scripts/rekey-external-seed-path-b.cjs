 ...
 *   - the re-key introduces no idx_catalog_products_source_identity collision
 *     (UNIQUE on merchant_id, platform, source_product_id).
 *
      ...
      updated AS (
        UPDATE pdp_identity_listing AS tgt
         ...
         WHERE c.source_product_id = tgt.product_id
           AND c.new_merchant_id = $4
           AND tgt.merchant_id = $1
        RETURNING tgt.product_id AS row_ref
      )
      INSERT INTO external_seed_rekey_ledger
        (batch_id, source_table, row_ref, old_merchant_id, new_merchant_id)
...
async function runPreflight(exec, sellerId = null) {
  ...
  if (againstExisting > 0) failures.push(`${againstExisting} rows collide with an existing (merchant_id, platform, source_product_id)`);
  if (withinBatch > 0) failures.push(`${withinBatch} duplicate keys within the batch itself`);
...
async function rollbackBatch(batchId) {
  const byTable = {
    catalog_products: 'UPDATE catalog_products SET merchant_id = $1 WHERE product_key = $2',
    catalog_skus: 'UPDATE catalog_skus SET merchant_id = $1 WHERE sku_key = $2',
    catalog_offers: 'UPDATE catalog_offers SET merchant_id = $1 WHERE product_key = $2',
    pdp_identity_listing: 'UPDATE pdp_identity_listing SET merchant_id = $1 WHERE product_id = $2',
  };
  await withClient(async (client) => {
    await client.query('BEGIN');
    try {
      const entries = await client.query(
        'SELECT source_table, row_ref, old_merchant_id FROM external_seed_rekey_ledger WHERE batch_id = $1',
        [batchId],
      );
      ...
      for (const e of entries.rows) {
        ...
        // eslint-disable-next-line no-await-in-loop
        const res = await client.query(sql, [e.old_merchant_id, e.row_ref]);
