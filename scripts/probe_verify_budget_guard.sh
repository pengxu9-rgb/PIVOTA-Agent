...
upload_json="$(curl -sS -X POST "$BASE/v1/photos/upload" \
...
for i in $(seq 1 "$CALLS"); do
  ...
  analysis_json="$(curl -sS -X POST "$BASE/v1/analysis/skin" \
...
curl -sS "$BASE/metrics" | awk '/^verify_calls_total\{/{print}'

echo "-- verify_fail_total labels --"
curl -sS "$BASE/metrics" | awk '/^verify_fail_total\{/{print}'

echo "-- verify_budget_guard_total --"
curl -sS "$BASE/metrics" | awk '/^verify_budget_guard_total/{print}'
