...
function buildMarker({ reason, evidence, sourceUrl, generatedAt }) {
  return {
...
function patchSeedData(seedData, marker) {
  ...
  next.content_evidence_hold_v1 = marker;
  snapshot.content_evidence_hold_v1 = marker;
...
async function countCatalogRows(ids) {
  if (!ids.length) return { products: 0, skus: 0, index_rows: 0 };
  const products = await query(`SELECT count(*)::int AS count FROM catalog_products WHERE source_product_id = ANY($1::text[])`, [ids]);
  const skus = await query(`SELECT count(*)::int AS count FROM catalog_skus WHERE source_product_id = ANY($1::text[])`, [ids]);
...
async function applyRows(rows, marker, { write }) {
  if (!write) return { external_product_seeds: 0, catalog_products: 0, catalog_skus: 0, index_pipeline_state: 0 };
  let seedUpdates = 0;
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const indexResult = await client.query(
          ...
          `,
          [externalProductId, marker.reason],
        );
...
async function main() {
  ...
  const reason = asString(argValue('reason', 'official_source_missing_editorial_content'));
  ...
  const marker = buildMarker({ reason, evidence, sourceUrl, generatedAt });
