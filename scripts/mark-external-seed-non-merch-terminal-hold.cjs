...
const HOLD_VERSION = 'external_seed.non_merch_terminal_hold.v1';
...
function markNonMerchCommerceHold(target) {
  if (!target || typeof target !== 'object' || Array.isArray(target)) return;
  deletePriceFields(target);
  target.product_kind = 'non_merch';
  target.product_family = 'non_merch';
  target.external_seed_product_family = 'non_merch';
...
function patchCommerceArrays(target) {
  ...
  for (const key of ['variants', 'product_variants', 'skus', 'offers']) {
    ...
    target[key] = target[key].map((item) => {
      ...
      markNonMerchCommerceHold(next);
      return next;
    });
  }
}

function buildContracts({ reason, evidence, sourceUrl, generatedAt }) {
  const hold = {
...
function patchSeedData(seedData, contracts) {
  ...
  markNonMerchCommerceHold(next);
  markNonMerchCommerceHold(snapshot);
  patchCommerceArrays(next);
  patchCommerceArrays(snapshot);

  next.non_merch_terminal_hold_v1 = contracts.hold;
  snapshot.non_merch_terminal_hold_v1 = contracts.hold;
  ...
  const contract = {
    ...
    updated_at: contracts.hold.updated_at,
  };
...
async function applyRows(rows, contracts, { write }) {
  ...
  const payload = {
    ...
    non_merch_terminal_hold_v1: contracts.hold,
...
async function main() {
  ...
  const reason = asString(argValue('reason', 'non_merchandise_promo_or_gift_card'));
  ...
  const contracts = buildContracts({ reason, evidence, sourceUrl, generatedAt });
