#!/usr/bin/env python3
"""Apply ingredient-family patch CSV to an ingredient workbook copy."""
...
try:
    ...
    raise SystemExit(
        "openpyxl is required to apply workbook ingredient-family patches. Install it in the local Python environment first."
    ) from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply ingredient-family patch CSV to a workbook copy.")
    parser.add_argument("--ingredient-xlsx", required=True, help="Source ingredient workbook")
    parser.add_argument("--patch-csv", required=True, help="Apply-ready family patch CSV")
    ...
    required_columns = {"record_id", "ingredient_family"}
    ...
    for patch in patch_rows:
        ...
        family_cell = sheet.cell(row=row_number, column=header_index["ingredient_family"])
        current_family = normalize_text(family_cell.value)
        expected_family = normalize_text(patch.get("existing_ingredient_family"))

        if current_family != expected_family:
            skipped_conflicts.append(
                {
                    "record_id": record_id,
                    "current_ingredient_family": current_family,
                    "expected_ingredient_family": expected_family,
                }
            )
            continue

        family_cell.value = normalize_text(patch.get("patch_ingredient_family"))
