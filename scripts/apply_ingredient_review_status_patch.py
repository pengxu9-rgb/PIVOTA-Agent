#!/usr/bin/env python3
"""Apply review_status patch CSV to an ingredient workbook copy."""
...
try:
    ...
    raise SystemExit(
        "openpyxl is required to apply workbook review_status patches. Install it in the local Python environment first."
    ) from exc
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Apply ingredient review_status patch CSV to a workbook copy.")
    parser.add_argument("--ingredient-xlsx", required=True, help="Source ingredient workbook")
    parser.add_argument("--patch-csv", required=True, help="Apply-ready review_status patch CSV")
    ...
    required_columns = {"record_id", "review_status"}
    ...
    for patch in patch_rows:
        ...
        status_cell = sheet.cell(row=row_number, column=header_index["review_status"])
        current_status = normalize_text(status_cell.value)
        expected_status = normalize_text(patch.get("existing_review_status"))
        ...
        status_cell.value = normalize_text(patch.get("patch_review_status"))
