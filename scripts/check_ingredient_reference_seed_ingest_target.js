...
function hasCoveringUniqueIndex(indexRows, columnName) {
  const needle = `(${String(columnName).toLowerCase()})`;
  return indexRows.some((row) => {
    const indexDef = String(row.indexdef || '').toLowerCase();
    return indexDef.includes('unique index') && indexDef.includes(needle);
  });
}
...
async function main() {
  ...
  const payload = {
    ...
    recommended_key_checks: {
      ...
      secondary_unique_columns_present: [],
      secondary_unique_indexes_detected: [],
    },
    indexes: [],
  };
  ...
  payload.recommended_key_checks.secondary_unique_columns_present = (
    manifest.recommended_secondary_unique_keys || []
  ).filter((column) => dbColumnSet.has(column));
  payload.recommended_key_checks.secondary_unique_indexes_detected = (
    manifest.recommended_secondary_unique_keys || []
  ).filter((column) => hasCoveringUniqueIndex(inspection.indexes, column));
