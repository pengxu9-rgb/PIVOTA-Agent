#!/usr/bin/env python3
"""Autofill signal-review packet decisions for selected signal buckets."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Autofill signal-review packet decisions for selected signal buckets.")
    parser.add_argument("--decision-csv", required=True, help="Input signal-review decision packet CSV")
    parser.add_argument("--out-csv", required=True, help="Output CSV with autofilled decisions")
    parser.add_argument(
        "--approve-bucket",
        action="append",
        default=[],
        help="Signal bucket to auto-mark as approve_suggestion. May be passed multiple times.",
    )
    parser.add_argument(
        "--only-empty-decision",
        action="store_true",
    ...
    for row in rows:
        bucket = normalize_text(row.get("suggested_signal_bucket"))
