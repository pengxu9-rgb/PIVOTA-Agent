...
preference-round1-pack:
	...
	node scripts/preference_round1_pack.mjs $(if $(RUN_ID),--run_id "$(RUN_ID)",) --review_in "$(REVIEW_JSONL)" --variant "$(if $(VARIANT),$(VARIANT),$(PREFERENCE_VARIANT))" --seed "$(if $(PREFERENCE_SEED),$(PREFERENCE_SEED),preference_round1_seed_v1)" --limit_internal "$(if $(LIMIT_INTERNAL),$(LIMIT_INTERNAL),38)" --limit_lapa "$(if $(LIMIT_LAPA),$(LIMIT_LAPA),60)" --limit_celeba "$(if $(LIMIT_CELEBA),$(LIMIT_CELEBA),60)" --internal_dir "$(if $(INTERNAL_DIR),$(INTERNAL_DIR),$(HOME)/Desktop/Aurora/internal test photos)" --cache_dir "$(CACHE_DIR)" --module_box_mode "$(if $(PREFERENCE_MODULE_BOX_MODE),$(PREFERENCE_MODULE_BOX_MODE),dynamic_skinmask)" --require_dynamic_boxes "$(if $(PREFERENCE_REQUIRE_DYNAMIC_BOXES),$(PREFERENCE_REQUIRE_DYNAMIC_BOXES),true)" --exclude_template_like "$(if $(PREFERENCE_EXCLUDE_TEMPLATE_LIKE),$(PREFERENCE_EXCLUDE_TEMPLATE_LIKE),false)" --min_geometry_qc_score "$(if $(PREFERENCE_MIN_GEOMETRY_QC_SCORE),$(PREFERENCE_MIN_GEOMETRY_QC_SCORE),0.2)" --template_match_eps "$(if $(PREFERENCE_TEMPLATE_MATCH_EPS),$(PREFERENCE_TEMPLATE_MATCH_EPS),0.004)" --hard_filter_gate "$(if $(PREFERENCE_HARD_FILTER_GATE),$(PREFERENCE_HARD_FILTER_GATE),true)" --hard_filter_require_quality_pass "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_QUALITY_PASS),$(PREFERENCE_HARD_FILTER_REQUIRE_QUALITY_PASS),false)" --hard_filter_max_guarded_modules "$(if $(PREFERENCE_HARD_FILTER_MAX_GUARDED_MODULES),$(PREFERENCE_HARD_FILTER_MAX_GUARDED_MODULES),1)" --hard_filter_min_module_pixels "$(if $(PREFERENCE_HARD_FILTER_MIN_MODULE_PIXELS),$(PREFERENCE_HARD_FILTER_MIN_MODULE_PIXELS),48)" --hard_filter_min_dynamic_score "$(if $(PREFERENCE_HARD_FILTER_MIN_DYNAMIC_SCORE),$(PREFERENCE_HARD_FILTER_MIN_DYNAMIC_SCORE),0.7)" --hard_filter_min_box_plausibility "$(if $(PREFERENCE_HARD_FILTER_MIN_BOX_PLAUSIBILITY),$(PREFERENCE_HARD_FILTER_MIN_BOX_PLAUSIBILITY),0.72)" --hard_filter_min_mask_rle_ratio "$(if $(PREFERENCE_HARD_FILTER_MIN_MASK_RLE_RATIO),$(PREFERENCE_HARD_FILTER_MIN_MASK_RLE_RATIO),0)" --hard_filter_min_face_span_h "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_H),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_H),0)" --hard_filter_min_face_span_w "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_W),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_W),0)" --hard_filter_min_face_span_area "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_AREA),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_AREA),0)" --hard_filter_require_onnx_skinmask "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_ONNX_SKINMASK),$(PREFERENCE_HARD_FILTER_REQUIRE_ONNX_SKINMASK),true)" --hard_filter_min_overlap_score "$(if $(PREFERENCE_HARD_FILTER_MIN_OVERLAP_SCORE),$(PREFERENCE_HARD_FILTER_MIN_OVERLAP_SCORE),0.6)" --hard_filter_max_abs_yaw "$(if $(PREFERENCE_HARD_FILTER_MAX_ABS_YAW),$(PREFERENCE_HARD_FILTER_MAX_ABS_YAW),0.85)" --hard_filter_require_all_strong_modules "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_ALL_STRONG_MODULES),$(PREFERENCE_HARD_FILTER_REQUIRE_ALL_STRONG_MODULES),true)" --hard_filter_fail_on_empty "$(if $(PREFERENCE_HARD_FILTER_FAIL_ON_EMPTY),$(PREFERENCE_HARD_FILTER_FAIL_ON_EMPTY),true)" --skinmask_onnx_enabled "$(if $(PREFERENCE_SKINMASK_ONNX_ENABLED),$(PREFERENCE_SKINMASK_ONNX_ENABLED),true)" --skinmask_onnx_strict "$(if $(PREFERENCE_SKINMASK_ONNX_STRICT),$(PREFERENCE_SKINMASK_ONNX_STRICT),true)" --skinmask_model_path "$(if $(PREFERENCE_SKINMASK_MODEL_PATH),$(PREFERENCE_SKINMASK_MODEL_PATH),artifacts/skinmask_v2.onnx)" --skinmask_timeout_ms "$(if $(PREFERENCE_SKINMASK_TIMEOUT_MS),$(PREFERENCE_SKINMASK_TIMEOUT_MS),1200)" --max_edge "$(MAX_EDGE)" --concurrency "$(EVAL_CONCURRENCY)" $(if $(OUT),--out "$(OUT)",) $(if $(MOCK_PIPELINE),--mock_pipeline "$(MOCK_PIPELINE)",)

preference-round1-real-pack:
	@test -n "$(INTERNAL_DIR)" || (echo "INTERNAL_DIR is required for real round1 pack" && exit 2)
	node scripts/preference_round1_real_runbook.mjs $(if $(RUN_ID),--run_id "$(RUN_ID)",) --internal_dir "$(INTERNAL_DIR)" --seed "$(if $(PREFERENCE_SEED),$(PREFERENCE_SEED),preference_round1_real_seed_v1)" --external_index_lapa "$(if $(EXTERNAL_INDEX_LAPA),$(EXTERNAL_INDEX_LAPA),$(CACHE_DIR)/lapa/index.jsonl)" --external_index_celeba "$(if $(EXTERNAL_INDEX_CELEBA),$(EXTERNAL_INDEX_CELEBA),$(CACHE_DIR)/celebamaskhq/index.jsonl)" --cache_dir "$(CACHE_DIR)" --limit_internal "$(if $(LIMIT_INTERNAL),$(LIMIT_INTERNAL),60)" --limit_lapa "$(if $(LIMIT_LAPA),$(LIMIT_LAPA),70)" --limit_celeba "$(if $(LIMIT_CELEBA),$(LIMIT_CELEBA),70)" --target_total "$(if $(TARGET_TOTAL),$(TARGET_TOTAL),200)" --overlap_ratio "$(if $(OVERLAP_RATIO),$(OVERLAP_RATIO),$(if $(IAA_RATIO),$(IAA_RATIO),0.25))" --overlap_min "$(if $(OVERLAP_MIN),$(OVERLAP_MIN),40)" --module_box_mode "$(if $(PREFERENCE_MODULE_BOX_MODE),$(PREFERENCE_MODULE_BOX_MODE),dynamic_skinmask)" --require_dynamic_boxes "$(if $(PREFERENCE_REQUIRE_DYNAMIC_BOXES),$(PREFERENCE_REQUIRE_DYNAMIC_BOXES),true)" --exclude_template_like "$(if $(PREFERENCE_EXCLUDE_TEMPLATE_LIKE),$(PREFERENCE_EXCLUDE_TEMPLATE_LIKE),false)" --min_geometry_qc_score "$(if $(PREFERENCE_MIN_GEOMETRY_QC_SCORE),$(PREFERENCE_MIN_GEOMETRY_QC_SCORE),0.2)" --template_match_eps "$(if $(PREFERENCE_TEMPLATE_MATCH_EPS),$(PREFERENCE_TEMPLATE_MATCH_EPS),0.004)" --hard_filter_gate "$(if $(PREFERENCE_HARD_FILTER_GATE),$(PREFERENCE_HARD_FILTER_GATE),true)" --hard_filter_require_quality_pass "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_QUALITY_PASS),$(PREFERENCE_HARD_FILTER_REQUIRE_QUALITY_PASS),false)" --hard_filter_max_guarded_modules "$(if $(PREFERENCE_HARD_FILTER_MAX_GUARDED_MODULES),$(PREFERENCE_HARD_FILTER_MAX_GUARDED_MODULES),1)" --hard_filter_min_module_pixels "$(if $(PREFERENCE_HARD_FILTER_MIN_MODULE_PIXELS),$(PREFERENCE_HARD_FILTER_MIN_MODULE_PIXELS),48)" --hard_filter_min_dynamic_score "$(if $(PREFERENCE_HARD_FILTER_MIN_DYNAMIC_SCORE),$(PREFERENCE_HARD_FILTER_MIN_DYNAMIC_SCORE),0.7)" --hard_filter_min_box_plausibility "$(if $(PREFERENCE_HARD_FILTER_MIN_BOX_PLAUSIBILITY),$(PREFERENCE_HARD_FILTER_MIN_BOX_PLAUSIBILITY),0.72)" --hard_filter_min_mask_rle_ratio "$(if $(PREFERENCE_HARD_FILTER_MIN_MASK_RLE_RATIO),$(PREFERENCE_HARD_FILTER_MIN_MASK_RLE_RATIO),0)" --hard_filter_min_face_span_h "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_H),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_H),0)" --hard_filter_min_face_span_w "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_W),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_W),0)" --hard_filter_min_face_span_area "$(if $(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_AREA),$(PREFERENCE_HARD_FILTER_MIN_FACE_SPAN_AREA),0)" --hard_filter_require_onnx_skinmask "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_ONNX_SKINMASK),$(PREFERENCE_HARD_FILTER_REQUIRE_ONNX_SKINMASK),true)" --hard_filter_min_overlap_score "$(if $(PREFERENCE_HARD_FILTER_MIN_OVERLAP_SCORE),$(PREFERENCE_HARD_FILTER_MIN_OVERLAP_SCORE),0.6)" --hard_filter_max_abs_yaw "$(if $(PREFERENCE_HARD_FILTER_MAX_ABS_YAW),$(PREFERENCE_HARD_FILTER_MAX_ABS_YAW),0.85)" --hard_filter_require_all_strong_modules "$(if $(PREFERENCE_HARD_FILTER_REQUIRE_ALL_STRONG_MODULES),$(PREFERENCE_HARD_FILTER_REQUIRE_ALL_STRONG_MODULES),true)" --hard_filter_fail_on_empty "$(if $(PREFERENCE_HARD_FILTER_FAIL_ON_EMPTY),$(PREFERENCE_HARD_FILTER_FAIL_ON_EMPTY),true)" --skinmask_onnx_enabled "$(if $(PREFERENCE_SKINMASK_ONNX_ENABLED),$(PREFERENCE_SKINMASK_ONNX_ENABLED),true)" --skinmask_onnx_strict "$(if $(PREFERENCE_SKINMASK_ONNX_STRICT),$(PREFERENCE_SKINMASK_ONNX_STRICT),true)" --skinmask_model_path "$(if $(PREFERENCE_SKINMASK_MODEL_PATH),$(PREFERENCE_SKINMASK_MODEL_PATH),artifacts/skinmask_v2.onnx)" --skinmask_timeout_ms "$(if $(PREFERENCE_SKINMASK_TIMEOUT_MS),$(PREFERENCE_SKINMASK_TIMEOUT_MS),1200)" --max_edge "$(if $(PREFERENCE_MAX_EDGE),$(PREFERENCE_MAX_EDGE),512)" --concurrency "$(EVAL_CONCURRENCY)" $(if $(OUT),--out "$(OUT)",) $(if $(REVIEW_PACK_JSONL),--review_pack_jsonl "$(REVIEW_PACK_JSONL)",$(if $(REVIEW_JSONL),--review_pack_jsonl "$(REVIEW_JSONL)",)) $(if $(MOCK_PIPELINE),--mock_pipeline "$(MOCK_PIPELINE)",)
