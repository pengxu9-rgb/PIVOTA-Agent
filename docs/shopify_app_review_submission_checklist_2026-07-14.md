...
1. **Route A — "Free to install" + usage-based Billing API charges (the intended path).** Shopify distinguishes plain **"Free"** (no charges ever — what we declare 07-14) from **"Free to install"** (install $0, "additional charges may apply"). Post-approval, convert the listing and bill commission through Shopify: merchant approves `appSubscriptionCreate` with a **usage line item + capped amount** (e.g., 10% of Pivota-attributed order value, capped $X/mo; cap raises need merchant re-approval), then each attributed conversion posts `appUsageRecordCreate` for its `take_amount_cents`. T7's per-order math maps 1:1 onto usage records — upstream (T2 closure, T6 rollup) unchanged; only the collection leg swaps Stripe-invoice → usage records for Shopify-channel merchants. Economics: Shopify's app-revenue share is applied automatically per Partner org — currently 0% on the first $1M/yr, 15% above (no separate opt-in registration; **verify current terms + eligibility against Shopify's live Partner docs before relying on this**, as platform terms shift). No merchant-facing payment processor is set up: Shopify bills the merchant on their own Shopify invoice and remits the developer's share via Partner payouts (bank/PayPal + W-8/W-9 required in Partner Dashboard → Settings → Payouts). The Billing API needs no additional access scopes — App A keeps its read-only posture. **Do NOT add billing before 07-14** — it changes the listing category and adds review surface; ship it as a post-approval pricing update.
