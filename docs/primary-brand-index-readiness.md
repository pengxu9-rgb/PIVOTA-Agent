...
## Canonical brand-match indexes

Two partial `catalog_products` indexes, `idx_catalog_products_canonical_brand_lower_v1` on `lower(brand)` and `idx_catalog_products_canonical_brand_compact_v1` on `regexp_replace(lower(brand), '[^a-z0-9]+', '', 'g')`, both `WHERE content_key IS NOT NULL AND brand IS NOT NULL`, serve the brand page's commerce-index lane (`fetchBrandScopedCanonicalCandidates`). Its brand match used to OR the two comparisons in one filter, which scanned all of `catalog_products` on every brand-page cache miss (~44ms in production). It is now a UNION of one branch per spelling, built from `src/services/canonicalBrandMatchSql.js`, the module the definitions read. `brand` is `varchar(255)`, so no prefix bound is needed for the btree key limit. Create them with the same procedure, run `ANALYZE catalog_products`, and confirm an `Index Cond` on each before deploying.
