...
| R3c | ✅ **Close the win loop** — buy-destination action reads brand_rollup.indexing_arc → live, dated phase (still-indexing w/ re-check date · past-window · not-minted) instead of static "get indexed" | portal | 🟢 merged + deployed | #95 (dcba21f) |
...
- 2026-06-20 — R3c SHIPPED (portal #95 → dcba21f), closing the win loop. The buy-destination panel's action was a static "get your canonical pages indexed". It now reads `brand_rollup.indexing_arc` (already computed backend-side, sibling of store_as_destination — no backend change) and renders the live phase: still-indexing → "N of M products in Google's 30-90d window; re-audit on/after {recheck_on_or_after}"; past-window → "should be indexed; now a content/SEO gap, not a wait"; none-minted → falls back to step-1 "get indexed". Dates derive from real `pivota_signature_minted_at`; cited-vs-retrieved discipline preserved. So R3a measures whether the store is the buy destination AND R3c's action points at the merchant's real, dated indexing progress — a closed loop. Deploy: portal auto-deploys main via Vercel. To verify live: a fresh reseller (Chydan) audit — the panel shows the dated action when canonical PDPs are minted, else the step-1 fallback.
