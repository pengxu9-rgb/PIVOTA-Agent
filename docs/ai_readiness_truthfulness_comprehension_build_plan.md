...
| P1b | ✅ **Collapse brand-action-per-SKU in dedup** — brand-level levers collapse across products; per-product kept. (Cross-format dupes = reconciliation #943, not dedup.) | backend | 🟢 merged + deployed | #945 (6b73b4cb) |
| P2a | ✅ **`in_progress` clarity** — static dot + 'Started' label (was a spinner implying a running job) | portal | 🟢 merged + deployed | #92 (1543d77) |
| P2b | ✅ **"What AI names instead"** — retitled '…instead of you'; 'named N×' frequency; action co-located inline | portal | 🟢 merged + deployed | #92 (1543d77) |
...
- 2026-06-18 — P1b SHIPPED (backend #945 -> 6b73b4cb). dedupe_pending_tasks now collapses brand-level-lever actions (indexing/general_recommendation/pivota_integration) across SKUs (drop product_key + strip the ' — {product}' suffix), so 'Index/Convert/schema x N SKUs' becomes one; per-product levers untouched. ANALYSIS NOTE: the cross-FORMAT dupes (URL vs catalog product_key) can't be bridged at the dedup layer (no shared string) — that's reconciliation's job (#943, on re-audit). Remaining: P2a (in_progress spinner), P2b ('names instead' action), Q (operator follow-tests).
- 2026-06-18 — P2a + P2b SHIPPED (portal #92 → Vercel 1543d77). in_progress now a static 'Started' chip (no spinner); 'What AI names instead of you' shows 'named N×' + an inline 'to win these…' action. PLAN COMPLETE (P0-P2 all shipped). Only Q (should Pivota BD operators run follow-tests for merchants?) remains — a product decision, not a build.
