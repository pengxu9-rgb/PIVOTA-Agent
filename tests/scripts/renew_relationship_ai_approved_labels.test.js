...
const REF_SETS = {
  activeSeedRefs: new Set(['ext_active_seed']),
  catalogRefs: new Set(['catalog_key_1']),
};
...
function fakeQueryFn({ rows = [], seeds = [], catalog = [], updateRowCount = 0, calls = [] } = {}) {
...
describe('renew-relationship-ai-approved-labels', () => {
  ...
  test('evaluateRenewalCandidates keeps rows that verify and skips the rest with reasons', () => {
    ...
    const { renewableIds, skipped, suppressionReasons } = evaluateRenewalCandidates(rows, REF_SETS);

    expect(renewableIds).toEqual(['ok_seed', 'ok_catalog', 'ok_need_anchor']);
    expect(skipped).toEqual({ suppressed: 1, anchor_unresolvable: 1, candidate_unresolvable: 1 });
  ...
  test('runRenewal apply with nothing renewable issues no UPDATE', async () => {
    const calls = [];
    const queryFn = fakeQueryFn({
    ...
    const report = await runRenewal({ apply: true, queryFn });
