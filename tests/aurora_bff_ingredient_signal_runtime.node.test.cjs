...
test('ingredient signal runtime: signal-like short text stays raw even if reviewed signal lookup is temporarily unavailable', async () => {
  routeInternals.__setGetBestIngredientSignalMatchForTest(async () => null);
  routeInternals.__setGetBestIngredientReferenceMatchForTest(async () => null);

  const ahaTarget = await routeInternals.extractIngredientLookupTargetFromText('AHA', 'EN');
  assert.equal(ahaTarget, 'AHA');

  const spfTarget = await routeInternals.extractIngredientLookupTargetFromText('SPF 30', 'EN');
  assert.equal(spfTarget, 'SPF 30');
});
