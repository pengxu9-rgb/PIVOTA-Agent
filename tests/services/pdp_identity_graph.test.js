describe('pdpIdentityGraph', () => {
  ...
  test('reviewed multi-offer merge candidate does not create retailer URL conflict in soft cluster', () => {
    const {
      buildIdentityListingFromProduct,
      _internals,
    } = require('../../src/services/pdpIdentityGraph');
    ...
    const officialListing = {
      ...buildIdentityListingFromProduct({
        merchantId: 'external_seed',
        productId: 'ext_rms_radiance_official_100ml',
        sourceKind: 'external_seed',
        product: {
          title: 'Radiance Lock Setting Mist',
          brand: 'RMS Beauty',
      ...
      variant_axes: { size: '100 ml' },
    };
    const retailerListing = buildIdentityListingFromProduct({
      merchantId: 'external_seed',
      productId: 'ext_dermstore_rms_radiance_conflict',
      sourceKind: 'external_seed',
      product: {
        title: 'RMS Beauty Radiance Lock Setting Mist 100ml',
        brand: 'RMS Beauty',
        ...
        seed_data: {
          ...
          multi_offer_merge_candidate: {
            status: 'approved',
            target_source_listing_ref: 'external_seed:ext_rms_radiance_official_100ml',
            match_basis: [
              'normalized_brand_match',
              'title_core_match',
              'size_axis_match',
    ...
    const clusteredRetailer = clustered.find(
      (item) => item.source_listing_ref === 'external_seed:ext_dermstore_rms_radiance_conflict',
    );

    expect(clusteredRetailer.identity_status).toBe('approved');
    expect(clusteredRetailer.review_required).toBe(false);
    expect(clusteredRetailer.review_reason_codes).not.toContain('conflicting_official_url');
  });
  ...
  test('maybeBuildLiveSyntheticPdp can skip external seed hydration for product-line members', async () => {
    jest.resetModules();
    process.env = {
      ...ORIGINAL_ENV,
      NODE_ENV: 'test',
    ...
    const queryFn = jest.fn(async (sql) => {
      ...
      if (normalizedSql.includes('sellable_item_group_id = $1')) return { rows: [sourceRow] };
      if (normalizedSql.includes('product_line_id = $1')) return { rows: [sourceRow, siblingRow] };
      if (/FROM\s+external_product_seeds\s+WHERE/i.test(normalizedSql)) return { rows: [] };
      return { rows: [] };
    });
    ...
    const result = await maybeBuildLiveSyntheticPdp({
      ...
      queryFn,
      hydrateLineMemberPayloads: false,
    });

    const externalSeedHydrationCalls = queryFn.mock.calls.filter(([sql]) =>
      /FROM\s+external_product_seeds\s+WHERE/i.test(String(sql || '').replace(/\s+/g, ' ')),
    );
    expect(externalSeedHydrationCalls).toHaveLength(2);
    expect(queryFn.mock.calls.some(([sql]) => String(sql || '').includes('product_line_id = $1'))).toBe(true);
    expect(result?.synthetic_product.product_line_options).toEqual([
