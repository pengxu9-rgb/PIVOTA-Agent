...
describe('Claude lane on Vertex AI (ADC transport)', () => {
  afterEach(() => {
    ...
    delete process.env.PIVOTA_AGENT_CENTER_ANTHROPIC_MODEL;
    delete process.env.PIVOTA_AGENT_CENTER_ANTHROPIC_VERTEX_REGION;
  ...
  test('constructs AnthropicVertex under the service ADC seam — no ANTHROPIC_API_KEY needed', () => {
    ...
    // Claude is served from specific Vertex regions, never the "global"
    // endpoint Gemini uses — the default must be a concrete region.
    expect(observed.region).toBe('us-east5');
