...
describe('/agent/shop/v1/invoke find_products_multi cache-first search', () => {
  ...
  test('accepts top-level payload query for find_products_multi', async () => {
    ...
    // The cross-merchant cache stage was removed (2026-09-26); the query goes to the primary search.
    let upstreamQuery = null;
    const upstreamSearch = nock('http://pivota.test')
      .get('/agent/v1/products/search')
      .query((q) => {
        upstreamQuery = String(q.query || '');
        return true;
      })
      .reply(200, {
        status: 'success',
    ...
    expect(upstreamQuery.toLowerCase()).toContain('ipsa');
  });
