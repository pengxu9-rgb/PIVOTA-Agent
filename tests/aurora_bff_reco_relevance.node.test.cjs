...
test('__internal: local external seed finish-fit sunscreen compare continues after raw precise rows', async () => {
  const { __internal } = loadRoutesFresh();
  const observedQueries = [];
  const makeSeedRow = ({ id, title, summary, stage }) => ({
    id,
      ...
      if (String(sql || '').includes('support_query_precise')) {
        return {
          rows: [
            ...
            makeSeedRow({
              id: 'touchup_stick',
              title: 'Matte Sunscreen Stick SPF 50',
              summary: 'A portable touch-up sunscreen stick.',
              stage: 'support_query_precise',
            }),
            makeSeedRow({
              id: 'tinted_cushion',
              title: 'Invisible Sun Cushion SPF 50',
              summary: 'A tinted sunscreen cushion for touch-ups.',
              stage: 'support_query_precise',
            }),
            makeSeedRow({
              id: 'body_spf',
              title: 'Oil-Free Body Sunscreen SPF 50',
              summary: 'A body sunscreen lotion.',
              stage: 'support_query_precise',
            }),
          ],
        };
  ...
  assert.match(observedQueries[0].sql, /support_query_precise/);
  assert.match(observedQueries[1].sql, /support_category_positive/);
  assert.equal(out.local_external_seed_stage_debug[0]?.continued_after_precise_stage, true);
  const productIds = out.products.map((row) => String(row.product_id || ''));
  assert.ok(productIds.includes('ext_skin1004_water_fit'));
  assert.ok(productIds.includes('ext_boj_aqua_fresh'));
});
...
test('__internal: local external seed sunscreen support broadens to authority category after precise form-fit miss', async () => {
  const { __internal } = loadRoutesFresh();
  const observedQueries = [];
  const out = await __internal.searchLocalExternalSeedProducts({
    query: 'spf fluid oily skin',
    ...
    queryFn: async (sql, params) => {
      ...
      if (/support_query_precise/.test(sqlText)) {
        return { rows: [] };
      }
      return {
        rows: [
          {
            id: '227',
            external_product_id: 'ext_support_daily_sunscreen_227',
            destination_url: 'https://example.com/products/lightweight-face-spf',
            canonical_url: 'https://example.com/products/lightweight-face-spf',
            domain: 'example.com',
            title: 'Lightweight Face Sunscreen SPF 50',
            image_url: 'https://example.com/products/lightweight-face-spf.jpg',
            price_amount: 28,
            price_currency: 'USD',
            availability: 'in_stock',
            ...
            seed_data: {
              derived: {
                recall: {
                  retrieval_title: 'Lightweight Face Sunscreen SPF 50',
                  retrieval_summary: 'A non-greasy daily sunscreen for oily skin.',
                  category: 'sunscreen',
                  vertical: 'skincare',
                },
              },
              snapshot: {
                title: 'Lightweight Face Sunscreen SPF 50',
                description: 'Daily face SPF with a lighter feel.',
                category: 'Sunscreen',
              },
            },
            updated_at: new Date().toISOString(),
  ...
  assert.match(observedQueries[1].sql, /attached_product_key\s+IS\s+NULL/i);
  assert.deepEqual(observedQueries[1].params[2], [
    'sunscreen',
    'spf',
    'sun care',
    'sun protection',
  ...
  assert.equal(out.local_external_seed_stage_debug[0]?.stage, 'support_query_precise');
  assert.equal(out.local_external_seed_stage_debug[0]?.row_count, 0);
  assert.equal(out.local_external_seed_stage_debug[1]?.stage, 'support_category_fit_broad');
  assert.equal(out.products[0]?.retrieval_match_stage, 'support_category_fit_broad');
  assert.equal(out.products[0]?.retrieval_role_id, 'daily_sunscreen');
});
...
test('__internal: framework pool prefers full-face daily SPF over portable stick for routine sunscreen support', async () => {
  ...
  const state = __internal.finalizeConcernFrameworkCandidatePools(
    [
      ...
      },
      {
        ...
        display_name: 'Round Lab [DEAL] Birch Moisturizing Sunscreen UVLock SPF 45+ Broad Spectrum',
        category: 'sunscreen',
        product_type: 'sunscreen',
        retrieval_source: 'external_seed',
        retrieval_query: 'sunscreen under makeup',
  ...
  const stick = state.hard_reject.find((entry) => entry?.product?.product_id === 'external_seed_sunscreen_first_wear_stick')?.product || null;
  assert.ok(stick);
  assert.equal(
    state.selected_recommendations.some((item) => item?.product_id === 'external_seed_sunscreen_first_wear_stick'),
    false,
  );
  assert.ok(Number(sunscreen.framework_rank_score || 0) > Number(stick.framework_rank_score || 0));
});

test('__internal: framework pool keeps thin SPF/PA daily sunscreen authority ahead of portable routine stick', async () => {
  const { __internal } = loadRoutesFresh();
  const targetContext = {
    framework_id: 'recofw_test_oily_support_thin_spf_pa_authority',
    primary_role_id: 'oil_control_treatment',
    request_text: 'im oily skin. what product should i buy?',
    semantic_plan: {
      routine_mode: 'routine_mix',
  ...
  const state = __internal.finalizeConcernFrameworkCandidatePools(
    [
      {
        product_id: 'catalog_oil_balance_thin_spf_anchor',
        merchant_id: 'merchant_catalog_oil_balance_thin_spf_anchor',
        brand: 'Clarity Lab',
        name: 'Oil Balance Serum',
        display_name: 'Clarity Lab Oil Balance Serum',
        category: 'serum',
      ...
      {
        product_id: 'external_seed_moisturizer_thin_spf_support',
        merchant_id: 'external_seed',
        brand: 'LightLab',
        name: 'Daily Balance Gel Cream',
        display_name: 'LightLab Daily Balance Gel Cream',
      ...
      {
        product_id: 'external_seed_daily_spf_thin_authority',
        merchant_id: 'external_seed',
        brand: 'Beauty of Joseon',
        ...
        retrieval_step: 'sunscreen',
        retrieval_role_id: 'daily_sunscreen',
        local_external_seed_role_fit_score: 0.18,
      },
      {
        product_id: 'external_seed_daily_spf_portable_stick',
        merchant_id: 'external_seed',
        brand: 'Supergoop',
        ...
        short_description: 'A clear sunscreen stick for touch-ups and portable reapplication.',
      },
    ],
    { targetContext },
  );
  ...
  const sunscreen = state.selected_recommendations.find((item) => item?.matched_role_id === 'daily_sunscreen') || null;
  assert.ok(sunscreen);
  assert.equal(sunscreen.product_id, 'external_seed_daily_spf_thin_authority');
  assert.equal(sunscreen.candidate_step, 'sunscreen');
  assert.equal(sunscreen.framework_semantic_fit, true);
  assert.equal(
    state.selected_recommendations.some((item) => item?.product_id === 'external_seed_daily_spf_portable_stick'),
    false,
  );
  const stickReject = state.hard_reject.find((entry) => entry?.product?.product_id === 'external_seed_daily_spf_portable_stick');
  assert.ok(stickReject);
  assert.equal(
    state.selected_recommendations.some((item) => item?.product_id === 'external_seed_daily_spf_portable_stick'),
    false,
  );
...
test('__internal: framework pool keeps thin daily sunscreen authority when retrieval role is absent but matched role is owned', async () => {
  const { __internal } = loadRoutesFresh();
  const targetContext = {
    framework_id: 'recofw_test_oily_support_thin_spf_missing_retrieval_role',
    primary_role_id: 'oil_control_treatment',
    request_text: 'im oily skin. what product should i buy?',
    semantic_plan: {
      routine_mode: 'routine_mix',
    ...
    framework_roles: [
      ...
      },
      {
        ...
        preferred_step: 'sunscreen',
        label: 'Daily sunscreen',
        query_terms: ['daily sunscreen', 'sunscreen under makeup'],
        fit_keywords: ['spf', 'lightweight', 'uv filters', 'non-greasy'],
        ingredient_hypotheses: ['UV filters'],
        product_type_hypotheses: ['sunscreen'],
      },
  ...
  const state = __internal.finalizeConcernFrameworkCandidatePools(
    [
      {
        product_id: 'catalog_oil_balance_missing_role_anchor',
        merchant_id: 'merchant_catalog_oil_balance_missing_role_anchor',
        brand: 'Clarity Lab',
        name: 'Oil Balance Serum',
        display_name: 'Clarity Lab Oil Balance Serum',
        category: 'serum',
      ...
      {
        product_id: 'external_seed_moisturizer_missing_role_support',
        merchant_id: 'external_seed',
        brand: 'LightLab',
        name: 'Daily Balance Gel Cream',
        display_name: 'LightLab Daily Balance Gel Cream',
      ...
      {
        product_id: 'external_seed_daily_spf_missing_role_authority',
        merchant_id: 'external_seed',
        brand: 'Beauty of Joseon',
        title: 'Relief Sun : Rice + Probiotics (SPF50+ PA++++)',
        retrieval_source: 'external_seed',
  ...
  const sunscreen = state.selected_recommendations.find((item) => item?.matched_role_id === 'daily_sunscreen') || null;
  assert.ok(sunscreen);
  assert.equal(sunscreen.product_id, 'external_seed_daily_spf_missing_role_authority');
  assert.equal(sunscreen.retrieval_role_id ?? null, null);
  assert.equal(sunscreen.candidate_step, 'sunscreen');
  assert.equal(sunscreen.framework_semantic_fit, true);
  assert.equal(
    state.hard_reject.some((entry) => entry?.product?.product_id === 'external_seed_daily_spf_missing_role_authority'),
    false,
  );
});
  ...
  try {
    ...
    assert.ok(
      calls.some((call) =>
        ...
        call.query === 'gel cream moisturizer' &&
...
test('__internal: beauty local handoff external stage uses local authority peer after backend timeout', async () => {
  ...
  const calls = [];
  const targetContext = {
    framework_id: 'recofw_test_oily_backend_timeout_local_peer',
    primary_role_id: 'oil_control_treatment',
    routine_mode: 'same_role_comparison',
  ...
  __internal.__setRouteDependencyOverridesForTest({
    ...
    searchLocalExternalSeedProducts: async (args = {}) => {
      ...
      return {
        ...
        products: [
          {
            product_id: 'local_oil_control_1',
            merchant_id: 'external_seed',
            brand: 'The Ordinary',
            name: 'Niacinamide 10% + Zinc 1%',
            display_name: 'The Ordinary Niacinamide 10% + Zinc 1%',
