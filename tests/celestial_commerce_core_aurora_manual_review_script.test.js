...
describe('Celestial commerce-core aurora manual review runner', () => {
  ...
  test('accepts a locked cache main path when direct supplement is unnecessary', async () => {
    const repoRoot = path.join(__dirname, '..');
    const scriptPath = path.join(
      repoRoot,
      'scripts',
      ...
      JSON.stringify(
        {
          semantic_cases: [
            {
              id: 'aurora_guidance_only_direct_supplement_manual',
              title: 'locked cache main path manual case',
              family: 'aurora_guidance_only_direct_supplement',
              execution_mode: 'manual',
              request: {
                operation: 'find_products_multi',
    ...
    const server = http.createServer(async (req, res) => {
      ...
      res.statusCode = 200;
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('X-Request-Id', 'aurora-manual-cache-main-path');
      res.end(
        JSON.stringify({
          products: [{ title: 'Winona Soothing Repair Serum' }],
          metadata: {
    ...
    try {
      ...
      expect(summary.results[0].notes).toContain('locked cache main path already satisfied the request');
      expect(Array.isArray(summary.results[0].checklist)).toBe(true);
      expect(summary.results[0].checklist).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            label: expect.stringContaining('Locked cache main path stayed authoritative'),
            status: 'pass',
          }),
        ]),
      );
