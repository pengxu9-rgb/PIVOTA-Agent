...
test('runRoutineAnalysisV2: routine audit v1 keeps stage A in a single chunk above the legacy split threshold', async () => {
  const { runRoutineAnalysisV2 } = require('../src/auroraBff/routineAnalysisV2');
  let stageACallCount = 0;
  const llmGateway = {
    async callWithSchemaDiagnostics({ templateId, params }) {
      if (templateId === 'routine_product_audit_v1') {
        stageACallCount += 1;
        const products = Array.isArray(params?.routine_products_json)
          ? params.routine_products_json
          : [];
        return {
          parsed: buildStageAResult(products.map((product) => buildAuditProduct(product.product_ref, {
            ...
            likelyRole: 'support step',
            likelyKeySignals: ['signal'],
          }))),
          parsedCandidate: null,
          raw: '{}',
          provider: 'stub',
  ...
  const result = await runRoutineAnalysisV2({
    requestId: 'req_stage_a_single_chunk_audit',
    language: 'EN',
    profileSummary: {
      skinType: 'combination',
      sensitivity: 'medium',
    ...
    routineProductCandidates: [
      { product_ref: 'routine_am_01', slot: 'am', step: 'cleanser', product_text: 'AM cleanser 1' },
      { product_ref: 'routine_am_02', slot: 'am', step: 'treatment', product_text: 'AM serum 1' },
      { product_ref: 'routine_am_03', slot: 'am', step: 'moisturizer', product_text: 'AM moisturizer 1' },
      { product_ref: 'routine_am_04', slot: 'am', step: 'spf', product_text: 'AM SPF 1' },
      { product_ref: 'routine_pm_05', slot: 'pm', step: 'cleanser', product_text: 'PM cleanser 1' },
      { product_ref: 'routine_pm_06', slot: 'pm', step: 'treatment', product_text: 'PM serum 1' },
      { product_ref: 'routine_pm_07', slot: 'pm', step: 'moisturizer', product_text: 'PM moisturizer 1' },
    ],
    llmGateway,
    recommendationResolverDeps: {
      resolveProduct: async () => null,
      searchProducts: async () => {
        throw new Error('routine_audit_v1 should skip recommendation resolution');
      },
    },
    surfaceMode: 'routine_audit_v1',
  });

  assert.equal(stageACallCount, 1);
