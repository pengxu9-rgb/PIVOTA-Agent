  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.equal(preview.payload && preview.payload.counts && preview.payload.counts.input_total, 5);
        assert.equal(preview.payload && preview.payload.counts && preview.payload.counts.total, 3);
        assert.equal(preview.payload && preview.payload.counts && preview.payload.counts.both, 2);
        const firstPreviewItem = preview.payload?.groups?.[0]?.items?.[0];
        const previewGroups = Array.isArray(preview?.payload?.groups) ? preview.payload.groups : [];
        const bothGroup = previewGroups.find((group) => group && group.slot === 'both');
        const previewItems = previewGroups.flatMap((group) => (Array.isArray(group?.items) ? group.items : []));
        const cleanserRows = previewItems.filter((item) => item && item.display_name === 'Biotherm Force Cleanser');
        const moisturizerRows = previewItems.filter((item) => item && item.display_name === 'Aquasource Hydra Barrier Cream');
        ...
        assert.ok(bothGroup, 'shared AM/PM products should be grouped once under slot=both');
        assert.equal(cleanserRows.length, 1, 'duplicated cleanser should only appear once');
        assert.equal(moisturizerRows.length, 1, 'duplicated moisturizer should only appear once');
        assert.deepEqual(cleanserRows[0]?.used_in, ['am', 'pm']);
        assert.deepEqual(moisturizerRows[0]?.used_in, ['am', 'pm']);
        assert.match(cleanserRows[0]?.suggested_usage?.reason || '', /both AM and PM/i);
