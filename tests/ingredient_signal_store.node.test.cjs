...
test('ingredient signal store: falls back to shared DATABASE_URL when signal-specific env vars are absent', async (t) => {
  await withEnv(
    {
      DATABASE_URL: 'postgres://app-db-only',
      INGREDIENT_SIGNAL_DATABASE_URL: undefined,
    ...
    async () => {
      ...
      await store.lookupIngredientSignalCandidates('Thiamidol');

      assert.equal(url, 'postgres://app-db-only');
      assert.equal(poolConfigs.length, 1);
      assert.equal(poolConfigs[0].connectionString, 'postgres://app-db-only');
    },
  );
});
