...
const CONFIRM_TOKEN = 'APPLY_WAVE13_MASAMI_QUALITY_SNAPSHOT_REPAIR';
...
function qualityForRow(row) {
  ...
  const brand = firstString(row.catalog_brand, seedData.brand, snapshot.brand);
  ...
  const ingredients = firstString(seedData.pdp_ingredients_raw, snapshot.pdp_ingredients_raw);
  const howToUse = firstString(seedData.pdp_how_to_use_raw, snapshot.pdp_how_to_use_raw);
  ...
  return {
    ...
    details: {
      ...
      problems,
      components,
      repair_source: 'wave13_masami_quality_snapshot_repair',
    ...
    const plans = rows.map((row) => {
      ...
      return {
        ...
        ingredients_length: firstString(
          asObject(row.seed_data).pdp_ingredients_raw,
          asObject(asObject(row.seed_data).snapshot).pdp_ingredients_raw,
        ).length,
        how_to_use_length: firstString(
          asObject(row.seed_data).pdp_how_to_use_raw,
          asObject(asObject(row.seed_data).snapshot).pdp_how_to_use_raw,
        ).length,
        content_quality_score: quality.content_quality_score,
