...
const CONFIRM_TOKEN = 'APPLY_WAVE12_APICEUTICALS_QUALITY_SNAPSHOT_REPAIR';
...
function qualityForRow(row) {
  ...
  const brand = firstString(row.catalog_brand, asObject(row.seed_data).brand, asObject(row.seed_data).snapshot?.brand);
  const description = firstString(
    row.catalog_description,
    asObject(row.seed_data).description,
    asObject(row.seed_data).pdp_description_raw,
    asObject(row.seed_data).snapshot?.description,
  );
  const category = firstString(row.catalog_product_type, asObject(row.seed_data).category, asObject(row.seed_data).product_type);
  ...
  return {
    ...
    rules_version: 'v1-lite-direct-pdp',
    model_version: 'none',
    details: {
      content_quality_score: score,
      model_readiness_score: 0,
      ...
      problems,
      components,
      repair_source: 'wave12_apiceuticals_quality_snapshot_repair',
...
async function applyRepair(client, rows) {
  ...
  try {
    for (const row of rows) {
      ...
      const servingEligible = quality.content_quality_score >= 60 && row.has_in_stock_offer === true;
      ...
      await client.query(
        ...
        [
          ...
          firstString(row.catalog_description, asObject(row.seed_data).description).length,
          'wave12_qs_repair_v1',
        ],
      );
      results.push({
  ...
  try {
    ...
    const plans = rows.map((row) => {
      ...
      return {
        ...
        description_length: firstString(row.catalog_description, asObject(row.seed_data).description).length,
        content_quality_score: quality.content_quality_score,
        rules_version: quality.rules_version,
        problems: quality.details.problems,
        would_be_serving_eligible: quality.content_quality_score >= 60 && row.has_in_stock_offer === true,
      };
    });
    const apply_results = write ? await applyRepair(client, rows) : [];
    const report = {
      ...
      summary: {
        ...
        inserted_quality_snapshots: apply_results.length,
        would_update_index_state: write ? 0 : rows.length,
        updated_index_state: apply_results.length,
