...
function buildMarkdown(summary, domainRows, recommendedRows, sourceGapRows, outDir) {
  const lines = [];
  lines.push('# Wave67 Markato-Only Expansion Candidate Rollup');
  lines.push('');
...
async function main() {
  ...
  const domains = unique(CURATED_MARKATO_DOMAINS.map(normalizeDomain))
    .filter((domain) => domain && !BLOCKED_DOMAINS.has(domain))
    .sort();
  const externalIds = [];
