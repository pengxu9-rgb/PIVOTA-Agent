...
function mountDiagnosisV2Routes(app, { logger, llmProvider }) {
  ...
  // ── POST /v1/diagnosis/blueprint-to-routine ──
  app.post('/v1/diagnosis/blueprint-to-routine', async (req, res) => {
    ...
    try {
      ...
      if (!blueprint || !blueprint.am_steps || !blueprint.pm_steps) {
      ...
      const routineSkeleton = {
        ...
        am: blueprint.am_steps.map((step, i) => ({
          position: i + 1,
          step_label: step,
          product: null,
          locked: false,
        })),
        pm: blueprint.pm_steps.map((step, i) => ({
          position: i + 1,
          step_label: step,
          product: null,
          locked: false,
  ...
  // ── POST /v1/diagnosis/route-action ──
  app.post('/v1/diagnosis/route-action', async (req, res) => {
    ...
    try {
      const { action_type, diagnosis_id, goal_profile, routine_blueprint } = req.body || {};
  ...
  // ── POST /v1/diagnosis/bind-checkin ──
    ...
    try {
      ...
      const validEvents = events
        ...
        .map((e) => ({
          ...
          diagnosis_id: e.diagnosis_id || null,
          user_id: userId,
      ...
      logger?.info(
        { count: validEvents.length, user: userId },
        'diagnosis v2 telemetry events received',
      );
