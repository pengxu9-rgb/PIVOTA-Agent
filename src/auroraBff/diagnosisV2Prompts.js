const { GOAL_PRESETS } = require('./diagnosisV2Schema');
...
function buildStage1Prompt({ goals, customInput, availableContext, isColdStart }) {
  ...
  const postProcedureRule =
    goals.includes('post_procedure_repair')
  ...
  return {
    ...
    system: [
      ...
      '    "constraints": string[]',
      '  },',
      '  "followup_questions": [{ "id": string, "question": string, "options": [{ "id": string, "label": string }] }],',
...
function buildStage2Prompt({
  ...
  const confidenceCap = isColdStart
    ? hasPhoto
      ? 0.6
      : 0.4
    : 1.0;

  const historySection = Array.isArray(previousDiagnoses) && previousDiagnoses.length > 0
    ? [
        '',
...
function buildStage3Prompt({
  ...
  const hasTravelActive =
    Array.isArray(travelPlans) && travelPlans.some((tp) => {
      if (!tp || !tp.end_date) return false;
      return new Date(tp.end_date) >= new Date();
    });
  ...
  const improvementSection = Array.isArray(missingDataDimensions) && missingDataDimensions.length > 0
    ? [
        '',
  ...
  return {
    ...
    system: [
      ...
      '- Routine blueprint: AM max 4 steps, PM max 4 steps. Use generic step names (e.g., "gentle cleanser", "barrier repair serum"), NOT specific brand/SKU names.',
      '- Do NOT mention specific product brands or SKU names unless the system explicitly allows it.',
      '- next_actions MUST be non-empty. Include at least one actionable CTA.',
      '- conflict_rules: list any ingredient/step conflicts (e.g., "Do not layer retinoid with AHA in PM").',
      travelRule,
      coldStartRule,
      historySection,
      improvementSection,
