...
function buildRoutineProductAuditPrompt() {
  return [
    ...
    'analysis_context_hard_json contains strong user-relevant context. analysis_context_soft_json contains supportive but uncertainty-bearing context. analysis_context_conflicts_json contains conflicts that must not be treated as confirmed fact.',
    'If explicit request/profile context conflicts with snapshot-derived context, explicit context wins.',
    'Stale or low-quality artifact signals should lower certainty, not increase specificity.',
    'Do not restate snapshot-derived context as if the user explicitly said it.',
    ...
    '10. If inferred_product_type is known, do not hide behind pure information-gap answers. Give a category-level provisional judgment even when exact INCI is missing.',
    '11. For cleanser, moisturizer, sunscreen, and basic hydrating serum-like products, explain the likely routine role and practical fit boundaries from category signals before mentioning uncertainty.',
    '12. Avoid boilerplate language such as "consistency is key", "support the barrier", or "focus on balance" unless tied to the named product and concrete context.',
    '[/HARD_RULES]',
    '',
    '[MISSING_DATA_POLICY]',
    ...
    '- If inferred_product_type is known but ingredients are not confirmed, keep the verdict cautious while still giving a provisional category-level reason.',
    ...
    '- Do not write fit_for_skin_type.reason, fit_for_goals.reason, fit_for_season_or_climate.reason, or concise_reasoning_en as "without knowing the ingredients...", "hard to assess", "need climate information to assess", or "routine basic" when a product type can still be inferred.',
    '[/FORBIDDEN_BEHAVIOR]',
    '',
    '[INPUT_CONTEXT]',
    ...
    'analysis_context_hard={{analysis_context_hard_json}}',
    'analysis_context_soft={{analysis_context_soft_json}}',
    'analysis_context_evidence={{analysis_context_evidence_json}}',
    'analysis_context_conflicts={{analysis_context_conflicts_json}}',
