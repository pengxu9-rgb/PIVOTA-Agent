...
function buildVisionCanonicalPrompt({ dto } = {}) {
  ...
  return {
    ...
    userPrompt: [
      ...
      '- limited: the face is visible but evidence is weak/noisy; keep fewer, stronger cues rather than guessing.',
      '- insufficient: blur, lighting, occlusion, framing, or absent cues prevent grounded extraction.',
      'hard constraints:',
      '- English-only canonical semantics.',
      '- Ignore user goal, locale, routine preference, and any non-image prior when extracting cues.',
      ...
      '- If pass-quality context is provided and visibility_status is sufficient, return at least 2 distinct grounded observations.',
      '- If pass-quality context is provided and evidence is weak, use limited with 1-2 grounded observations only.',
      ...
      'Example B -> visibility_status=limited; observations=[{cue:"texture",region:"chin",...}] when only one weak but grounded cue is visible.',
      'Example C -> visibility_status=insufficient; insufficient_reason=blur; observations=[].',
    ].join('\n'),
  };
}
...
function buildReportCanonicalPrompt({ dto } = {}) {
  ...
  return {
    ...
    userPrompt: [
      'objective:',
      '- Convert the provided skin signals into a grounded, conservative report core.',
      '- Stay fully consistent with visible cues and deterministic signals.',
      '- Return only the fields needed for downstream deterministic plan construction.',
      'decision rubric:',
      ...
      '- Return only needs_risk_check, summary_focus, and insights.',
      '- insights must stay compact and grounded; return exactly 1-2 strongest cues, not broad coverage.',
      'hard constraints:',
      '- English-only canonical semantics.',
      '- No user-facing prose paragraphs and no free-form product advice.',
      '- Do not generate routine_steps, watchouts, follow_up, deepening, or localized copy.',
      '- If cues are too weak, do not invent confidence or claims.',
      '- Keep the output compact; evidence must be a short phrase, not a sentence paragraph.',
      '- When concern_rank or deterministic_signals point to a stronger priority, avoid falling back to mixed without evidence.',
      'compact context block:',
      contextJson,
      'few-shot mini examples:',
      'Example A -> barrier/redness priority with 2 grounded insights and no extra fields.',
      'Example B -> oiliness/texture priority with compact insights only; downstream rules will derive the routine.',
    ].join('\n'),
  };
}
...
function buildDeepeningCanonicalPrompt({ dto } = {}) {
  ...
  return {
    ...
    systemInstruction:
      'You are a structured skincare deepening phase selector. Reason in English only. Output strict JSON matching the response schema. Do not localize and do not produce long prose.',
    userPrompt: [
      'objective:',
