...
const BEAUTY_PLAN_CARD_SPECS = {
  pivot_beauty_contract: { type: 'routine', CN: '护肤建议', EN: 'Skincare guidance' },
  routine_minimal_plan: { type: 'routine', CN: '极简 routine', EN: 'Minimal routine' },
  beauty_capsule_plan: { type: 'routine', CN: '通勤 capsule', EN: 'Commute capsule' },
  travel_repair_kit_plan: { type: 'travel', CN: '旅行修护包', EN: 'Travel repair kit' },
  routine_audit_plan: { type: 'routine', CN: 'Routine 优先级', EN: 'Routine priority' },
  barrier_safety_plan: { type: 'routine', CN: '屏障修护优先', EN: 'Barrier first' },
  routine_safety_plan: { type: 'routine', CN: '活性分频建议', EN: 'Active schedule' },
};

function getBeautyPlanCardSpec(type, language = 'EN') {
  const row = BEAUTY_PLAN_CARD_SPECS[String(type || '').trim().toLowerCase()];
  if (!row) return null;
  return {
    type: row.type,
    title: language === 'CN' ? row.CN : row.EN,
  };
}
...
function buildPassthroughCard({ card, requestId, index, language = 'EN', fallbackTitle = '', fallbackType = '' }) {
  const payload = isPlainObject(card && card.payload) ? card.payload : {};
  const type = (asString(fallbackType) || asString(card && card.type)).toLowerCase();
  const title =
...
function mapLegacyCardToSpecCards(card, { requestId, language = 'EN', index = 0 } = {}) {
  ...
  const beautyPlanSpec = getBeautyPlanCardSpec(type, language);
  if (beautyPlanSpec) {
    return [
      buildPassthroughCard({
        card,
        requestId,
        index,
        language,
        fallbackTitle: beautyPlanSpec.title,
        fallbackType: beautyPlanSpec.type,
      }),
    ];
  }
