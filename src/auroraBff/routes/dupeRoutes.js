function ensureFunction(name, value) {
  ...
  throw new Error(`aurora dupe routes missing dependency: ${name}`);
}

function ensureSchema(name, value) {
  if (value && typeof value.safeParse === 'function') return value;
  throw new Error(`aurora dupe routes missing schema: ${name}`);
}

function mountDupeRoutes(app, deps = {}) {
  const applyDupeSuggestSanitizeToEnvelope = ensureFunction('applyDupeSuggestSanitizeToEnvelope', deps.applyDupeSuggestSanitizeToEnvelope);
  ...
  const extractAnchorIdFromProductLike = ensureFunction('extractAnchorIdFromProductLike', deps.extractAnchorIdFromProductLike);
  const extractJsonObjectByKeys = ensureFunction('extractJsonObjectByKeys', deps.extractJsonObjectByKeys);
  const fetchRecoAlternativesForProduct = ensureFunction('fetchRecoAlternativesForProduct', deps.fetchRecoAlternativesForProduct);
  const getDupeDeepscanCache = ensureFunction('getDupeDeepscanCache', deps.getDupeDeepscanCache);
  const getDupeKbEntry = ensureFunction('getDupeKbEntry', deps.getDupeKbEntry);
  ...
  const mapAuroraAlternativesToDupeCompare = ensureFunction('mapAuroraAlternativesToDupeCompare', deps.mapAuroraAlternativesToDupeCompare);
  const mapAuroraProductAnalysis = ensureFunction('mapAuroraProductAnalysis', deps.mapAuroraProductAnalysis);
  const mergeFieldMissing = ensureFunction('mergeFieldMissing', deps.mergeFieldMissing);
  const normalizeDupeCompare = ensureFunction('normalizeDupeCompare', deps.normalizeDupeCompare);
  const normalizeDupeKbKey = ensureFunction('normalizeDupeKbKey', deps.normalizeDupeKbKey);
  ...
  const setDupeDeepscanCache = ensureFunction('setDupeDeepscanCache', deps.setDupeDeepscanCache);
  const summarizeProfileForContext = ensureFunction('summarizeProfileForContext', deps.summarizeProfileForContext);
  const upsertDupeKbEntry = ensureFunction('upsertDupeKbEntry', deps.upsertDupeKbEntry);
  const DupeCompareRequestSchema = ensureSchema('DupeCompareRequestSchema', deps.DupeCompareRequestSchema);
  const DupeSuggestRequestSchema = ensureSchema('DupeSuggestRequestSchema', deps.DupeSuggestRequestSchema);

  const logger = deps && typeof deps.logger === 'object' ? deps.logger : null;
  const AURORA_DECISION_BASE_URL = deps.AURORA_DECISION_BASE_URL;
  const AURORA_DUPE_SUGGEST_SANITIZE_V1 = deps.AURORA_DUPE_SUGGEST_SANITIZE_V1 === true;
  const DUPE_KB_ASYNC_BACKFILL_ENABLED = deps.DUPE_KB_ASYNC_BACKFILL_ENABLED === true;
