...
const { buildStage1Prompt, buildStage2Prompt, buildStage3Prompt, PROMPT_VERSION } = require('./diagnosisV2Prompts');
const { DiagnosisV2IntroPayload, DiagnosisV2ResultPayload, validateResultPayload, ThinkingStepEvent } = require('./diagnosisV2Schema');
const { getLatestDiagnosisArtifact, saveDiagnosisArtifact } = require('./diagnosisArtifactStore');
...
function detectColdStart(ctx) {
  ...
  const hasRoutine = ctx.currentRoutine && ctx.currentRoutine !== 'none';
...
function compactDiagnosisSummary(artifact) {
  ...
  return {
    ...
    inferred_axes_summary: (data.inferred_state?.axes || []).map((ax) => ({
      axis: ax.axis,
      level: ax.level,
      confidence: ax.confidence,
    })),
    strategies_summary: (data.strategies || []).map((s) => s.title),
...
function checkLoginGate(ctx) {
  ...
  if (isLoggedIn) return { needsLogin: false };
  return {
    needsLogin: true,
    loginPromptPayload: {
...
async function runStage1({ goals, customInput, ctx, llmProvider }) {
  ...
  if (ctx.profile) availableContext.profile = ctx.profile;
  ...
  const introPayload = {
    goal_profile: {
      ...
      ...(parsed.goal_profile?.post_procedure_meta ? { post_procedure_meta: parsed.goal_profile.post_procedure_meta } : {}),
    },
    is_cold_start: isColdStart,
    question_strategy: isColdStart ? 'state_probe' : 'default',
    followup_questions: (parsed.followup_questions || []).slice(0, 3).map((q, i) => ({
      id: q.id || `fq_${i}`,
      question: q.question,
      options: (q.options || []).map((opt, j) => ({
        id: opt.id || `opt_${i}_${j}`,
        label: opt.label,
...
async function runStage2({ goalProfile, followupAnswers, photoFindings, ctx, llmProvider, onThinkingStep }) {
  ...
  if (ctx.userId) {
    try {
      const history = await getDiagnosisHistory(ctx.userId, { limit: 3 });
      previousDiagnoses = history.map(compactDiagnosisSummary).filter(Boolean);
...
// ── Stage 3: Plan Strategy & Blueprint ──
...
async function runDiagnosisV2({
  ...
  if (onThinkingStep) {
    onThinkingStep({ stage: 'goal_understanding', step: 'start', text: ctx.language === 'CN' ? '正在理解你的护肤目标...' : 'Understanding your skincare goals...', status: 'in_progress' });
  }

  const stage1 = await runStage1({ goals, customInput, ctx, llmProvider });

  if (onThinkingStep) {
    onThinkingStep({ stage: 'goal_understanding', step: 'done', text: ctx.language === 'CN' ? `已理解目标：${goals.join('、')}` : `Goals understood: ${goals.join(', ')}`, status: 'done' });
    onThinkingStep({ stage: 'inference', step: 'start', text: ctx.language === 'CN' ? '正在分析你的皮肤状态...' : 'Analyzing your skin state...', status: 'in_progress' });
  }
  ...
  if (onThinkingStep) {
    onThinkingStep({ stage: 'inference', step: 'done', text: ctx.language === 'CN' ? '皮肤状态分析完成' : 'Skin state analysis complete', status: 'done' });
    onThinkingStep({ stage: 'strategy', step: 'start', text: ctx.language === 'CN' ? '正在制定个性化策略...' : 'Creating your personalized strategy...', status: 'in_progress' });
  }
  ...
  if (onThinkingStep) {
    onThinkingStep({ stage: 'strategy', step: 'done', text: ctx.language === 'CN' ? '策略制定完成' : 'Strategy complete', status: 'done' });
  }
  ...
  if (ctx.userId) {
    ...
    } catch (_) { /* persist failure should not break the flow */ }
  }
...
async function getDiagnosisHistory(userId, { limit = 3 } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < limit; i++) {
      const artifact = await getLatestDiagnosisArtifact(userId, {
        schema: 'aurora.skin_diagnosis.v2',
        offset: i,
      });
      if (!artifact) break;
