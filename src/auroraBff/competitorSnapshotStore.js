...
function sanitizeSnapshotPayload(payload) {
  ...
  return {
    ...
    ...(Array.isArray(payload.competitor_queries)
      ? {
          competitor_queries: payload.competitor_queries
            .map((x) => String(x || '').trim())
            .filter(Boolean)
            .slice(0, 8),
        }
...
function sanitizeSnapshotMeta(meta, payload) {
  ...
  return {
    ...
    ranker_version:
      String(src.ranker_version || src.rankerVersion || 'reco_blocks_dag.v1').trim() ||
      'reco_blocks_dag.v1',
    ...
    reason_flags: Array.from(
      new Set(
        (Array.isArray(src.reason_flags) ? src.reason_flags : [])
...
function scoreForReplace(meta = {}) {
  ...
  const freshnessBoost =
    ageMs == null ? 0.05 : Math.max(0, Math.min(0.1, (HARD_TTL_MS - Math.max(0, ageMs)) / HARD_TTL_MS));
...
function writeSnapshot(key, payload, meta = {}) {
  ...
  if (existing && isPlainObject(existing)) {
    ...
    const confidenceImproved =
      normalizeConfidence(safeMeta.confidence) > normalizeConfidence(existing?.meta?.confidence);
    const newer =
      Number.isFinite(nextCreatedMs) && Number.isFinite(prevCreatedMs) ? nextCreatedMs >= prevCreatedMs : true;
