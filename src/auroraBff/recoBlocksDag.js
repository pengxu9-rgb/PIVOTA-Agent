...
const { scheduleDetachedAsyncJob } = require('./detachedAsyncRegistry');
const {
...
async function recoBlocks(anchor, ctx = {}, budgetMs = DEFAULT_BUDGET_MS) {
  ...
  if (!relPool.length && onPageMode === 'fallback_only') {
    ...
    if (onPageResult.candidates.length || onPageResult.related_products.length) {
      ...
      const routedRelatedPool = Array.isArray(onPageRouted?.rel_pool)
        ? onPageRouted.rel_pool
        : [];
      const adoptedRelatedPool =
        routedRelatedPool.length > 0
          ? routedRelatedPool
          : onPageCandidates;
      relPool = dedupeCandidates(
        [...relPool, ...adoptedRelatedPool],
