...
function buildMediaItems(product, variants) {
  ...
  const seen = new Set();
  const pushItem = (item) => {
    const url = normalizePdpImageUrl(item?.url);
    if (!url || isDecorativePdpMediaUrl(url)) return;
...
function resolveProductDescriptionText(product, detailSections = collectStructuredDetailSections(product)) {
  ...
  if (explicitPdpDescription && !looksLikeSectionSoupText(explicitPdpDescription)) {
  ...
  const cleanedPdpDescription = cleanOverviewDescriptionText(explicitPdpDescription);
