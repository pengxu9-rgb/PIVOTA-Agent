const { z } = require('zod');
const { RUNTIME_OPERATIONS } = require('./commerce/operationCatalog');

const OperationEnum = z.enum(RUNTIME_OPERATIONS);
...
const InvokeRequestSchema = z.object({
  ...
  payload: z.object({
    ...
    // create_order / confirm_payment: { order_id?, items[], shipping_address, notes? }
    order: z.any().optional(),

    // submit_payment: { order_id, expected_amount, currency, payment_method_hint?, return_url? }
    payment: z.any().optional(),
    
    // get_order_status / request_after_sales: { order_id, requested_action? (refund|cancel), reason? }
