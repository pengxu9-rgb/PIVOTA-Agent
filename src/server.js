...
function normalizeAuthoritativeSearchNoFallbackResponse(
  body,
  {
...
function isCreatorCheckoutSource(source) {
  return LEGACY_CREATOR_CHECKOUT_SOURCES.has(normalizeAgentSource(source));
}

function resolveCanonicalCreatorCheckoutSource({
  payload = {},
  payment = {},
  order = {},
  quote = {},
  ...
  const candidateSources = [
    payment?.source,
    payment?.request_context?.channel,
    order?.source,
    order?.request_context?.channel,
    orderMetadata?.source,
    quote?.source,
    quote?.request_context?.channel,
    payload?.source,
    context?.channel,
    metadata?.source,
    clientChannel,
  ...
  return candidateSources.some((candidate) => isCreatorCheckoutSource(candidate))
    ? CANONICAL_CREATOR_CHECKOUT_SOURCE
    : null;
}
...
function isCreatorInvokeSource(source) {
  const normalized = normalizeAgentSource(source);
  return normalized === 'creator-agent' || normalized === 'creator-agent-ui';
}
...
function hasExplicitLegacyInvokeContracts(metadata = null) {
...
function isPublicBeautyMainlineSearchRoute(metadata = null) {
  ...
  return (
    ...
    String(searchRequestContract.operation || '').trim() === 'find_products_multi' &&
    String(searchRequestContract.primary_lane || '').trim() === 'beauty_discovery_mainline'
  );
}

function isPublicSearchRouteMetadata(metadata = null) {
  return Boolean(
    metadata &&
      typeof metadata === 'object' &&
      !Array.isArray(metadata) &&
      metadata.public_search_route === true,
  );
}
...
function classifyInvokeSearchRail(source, { explicitLegacy = false, metadata = null } = {}) {
  ...
  if (
    isPublicSearchSource(source) ||
    isPublicSearchRouteMetadata(metadata) ||
    isPublicBeautyMainlineSearchRoute(metadata)
  ) {
...
function resolveInvokeSearchRailFromMetadata(metadata = null) {
  return classifyInvokeSearchRail(metadata?.source, {
    explicitLegacy: hasExplicitLegacyInvokeContracts(metadata),
    metadata,
  });
}

function buildInvokeSearchRailMetadata(searchRail = null) {
  const normalizedSearchRail = String(searchRail || '').trim().toLowerCase() || 'legacy_internal';
  return {
    invoke_search_rail: normalizedSearchRail,
    legacy_contract: normalizedSearchRail === 'legacy_internal',
  };
}
...
function resolvePublicSearchRouteSessionId({ req, query = null, metadata = null } = {}) {
  const queryParams =
  ...
  const headerSessionId =
    req && typeof req.header === 'function'
      ? req.header('x-session-id') || req.header('X-Session-Id')
      : null;
  return (
    String(
      firstQueryParamValue(queryParams.session_id || queryParams.sessionId) ||
...
function resolveLegacyBeautyCacheOwnerBypassForPublicSearchRoute() {
  return {
    bypass: false,
...
const agentProductsSearchRouteEntryRuntime = createFindProductsSearchRouteEntryRuntime({
  resolveGuidanceSearchSessionId: resolvePublicSearchRouteSessionId,
...
function shouldUseCreatorBeautyMainlineInternalPrimitive({
  operation = '',
  search = null,
  metadata = null,
  rawUserQuery = '',
  ...
  if (
    String(searchObj.category || '').trim() ||
    searchObj.price_min != null ||
    searchObj.min_price != null ||
    searchObj.price_max != null ||
...
function buildBeautyMainlineInternalPrimitiveSearchBody({
  search = null,
  queryParams = null,
  metadata = null,
  gatewayRequestId = null,
  ...
  const semanticContract =
    searchRequestContract?.semantic_contract &&
    typeof searchRequestContract.semantic_contract === 'object' &&
    !Array.isArray(searchRequestContract.semantic_contract)
      ? searchRequestContract.semantic_contract
      : {};
  const validation = sanitizeInternalProductsSearchRequest(
    {
      query: firstNonEmptyString(queryObj.query, searchObj.query, searchObj.q),
      limit: queryObj.limit ?? searchObj.limit ?? searchObj.page_size,
      offset: queryObj.offset,
      merchant_id: firstNonEmptyString(
        queryObj.merchant_id,
        searchObj.merchant_id,
        searchObj.merchantId,
      ),
      merchant_ids:
        queryObj.merchant_ids ?? searchObj.merchant_ids ?? searchObj.merchantIds,
      search_all_merchants:
        queryObj.search_all_merchants ??
        searchObj.search_all_merchants ??
        searchObj.searchAllMerchants,
      catalog_surface: firstNonEmptyString(
      ...
      in_stock_only:
        queryObj.in_stock_only ??
        searchObj.in_stock_only ??
        searchObj.inStockOnly,
      allow_external_seed:
        queryObj.allow_external_seed ??
        searchObj.allow_external_seed ??
        searchObj.allowExternalSeed,
      external_seed_strategy: firstNonEmptyString(
        queryObj.external_seed_strategy,
      ...
      query_step_strength: firstNonEmptyString(
        ...
        semanticContract?.query_step_strength,
      ),
      product_only:
        searchObj.product_only ??
        searchObj.productOnly,
  ...
  return validation.ok ? validation.search : null;
}
...
function isLegacyBeautyMainlineInvokeMetadata(metadata = null) {
  if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) return false;
  if (hasExplicitLegacyInvokeContracts(metadata)) return false;
  if (!(isAuroraSource(metadata.source) || isCreatorInvokeSource(metadata.source))) return false;
...
async function handleAgentProductsSearchViaInvoke(req, res) {
  ...
  if (!routePreparation || routePreparation.invalid || !routePreparation.payload) {
  ...
  const payload = {
    ...routePreparation.payload,
    metadata:
      routePreparation.payload?.metadata &&
      typeof routePreparation.payload.metadata === 'object' &&
      !Array.isArray(routePreparation.payload.metadata)
        ? {
            ...routePreparation.payload.metadata,
...
async function handleInvokeRequest(req, res, routeContext = {}) {
        ...
        case 'find_similar_products': {
          ...
          const excludeItems = Array.isArray(sim?.exclude_items)
            ...
            : Array.isArray(payload?.exclude_items)
              ...
              : Array.isArray(sim?.exclude_ids)
                ? sim.exclude_ids.map((excludedId) => ({ product_id: String(excludedId || '').trim() }))
                : [];
          const recentViews = Array.isArray(simContext?.recent_views)
            ? simContext.recent_views
            : Array.isArray(payloadContext?.recent_views)
              ? payloadContext.recent_views
              : Array.isArray(payload?.recent_views)
          ...
          const baseProduct =
            (productId ? getProductById(merchantId || DEFAULT_MERCHANT_ID, productId) : null) ||
            {
              merchant_id: merchantId || DEFAULT_MERCHANT_ID,
              product_id: productId,
            };
          const rec = productId
            ? await recommendPdpProducts({
            ...
            : { items: [], metadata: null, debug: null, cache: null };
          const picked = Array.isArray(rec?.items) ? rec.items : [];
          ...
          if (debugEnabled) {
            mockResponse.debug = rec?.debug || null;
            mockResponse.cache = rec?.cache || null;
          }
          break;
        }
        ...
        case 'get_product_detail': {
          ...
          if (pdpOptions.includeRecommendations) {
	        ...
	        case 'get_pdp_v2': {
	          ...
	          const merchantId = String(
	            productRef.merchant_id ||
	              productRef.merchantId ||
	              payload.merchant_id ||
	              payload.merchantId,
	          ).trim() || inferMerchantIdFromProductId(productId) || DEFAULT_MERCHANT_ID;
  ...
  try {
    ...
    // from products_cache (same source as creator categories / merchant portal).
      ...
      let upstreamMethod = route.method;
      let searchPrimaryContract = null;
	    let requestBody = {};
    ...
    switch (operation) {
      case 'find_products': {
        ...
        if (invokeSearchRail === 'legacy_internal') {
          ...
          searchPrimaryContract = 'agent_v2';
        }
        break;
      }
      ...
      case 'find_products_multi': {
        ...
        if (strictCommerceFindProductsMulti) {
          ...
          searchPrimaryContract = 'shop_invoke_strict';
        } else if (useCreatorBeautyMainlineInternalPrimitive) {
          const searchExecutionPlan = resolveFindProductsSearchExecutionPlan({
            requestContract: getSearchRequestContractFromMetadata(metadata),
            pivotaApiBase: PIVOTA_API_BASE,
            searchInvokeBase,
          });
          const internalPrimitiveSearchBody =
            buildBeautyMainlineInternalPrimitiveSearchBody({
              search,
              queryParams,
              metadata,
              gatewayRequestId,
          ...
          if (searchExecutionPlan?.upstream_url && internalPrimitiveSearchBody) {
            url = searchExecutionPlan.upstream_url;
            upstreamMethod = searchExecutionPlan.upstream_method || 'POST';
            requestBody = internalPrimitiveSearchBody;
        ...
        } else if (invokeSearchRail === 'legacy_internal') {
          url = `${searchInvokeBase}/agent/v1/products/search`;
          upstreamMethod = 'GET';
          searchPrimaryContract = 'agent_v1';
        } else {
    ...
    logger.info({ operation, method: upstreamMethod, url, hasQuery: Object.keys(queryParams).length > 0 }, 'Forwarding invoke request');
    ...
    const axiosConfig = {
      ...
      ...(upstreamMethod !== 'GET' && Object.keys(requestBody).length > 0 && { data: requestBody })
    };
    const buildFindProductsMultiSecondStageAxiosConfig = ({
      expandedQuery,
      limit,
    } = {}) => {
      ...
      const normalizedLimit = Math.min(
        Math.max(1, Number(limit || queryParams?.limit || 20) || 20),
        80,
      );
      if (!normalizedExpandedQuery) return null;
      const secondaryQueryParams = {
        ...queryParams,
        query: normalizedExpandedQuery,
        offset: 0,
        limit: normalizedLimit,
      };
      if (searchPrimaryContract === 'agent_v1') {
        return {
          method: 'GET',
      ...
      if (searchPrimaryContract === 'agent_v2') {
        const baseSearch =
        ...
        const secondarySearch = {
          ...baseSearch,
          ...(secondaryQueryParams?.merchant_id
            ? { merchant_id: secondaryQueryParams.merchant_id }
            : {}),
          ...(Array.isArray(secondaryQueryParams?.merchant_ids) &&
          secondaryQueryParams.merchant_ids.length > 0
            ? { merchant_ids: secondaryQueryParams.merchant_ids }
            : {}),
          ...(secondaryQueryParams?.search_all_merchants === true
            ? { search_all_merchants: true }
            : {}),
          query: normalizedExpandedQuery,
          limit: normalizedLimit,
          page: 1,
          ...(secondaryQueryParams?.category ? { category: secondaryQueryParams.category } : {}),
          ...(secondaryQueryParams?.catalog_surface
            ? { catalog_surface: secondaryQueryParams.catalog_surface }
            : {}),
          ...(secondaryQueryParams?.min_price != null
            ? { min_price: secondaryQueryParams.min_price }
            : {}),
          ...(secondaryQueryParams?.max_price != null
            ? { max_price: secondaryQueryParams.max_price }
            : {}),
          ...(secondaryQueryParams?.allow_external_seed !== undefined
            ? { allow_external_seed: secondaryQueryParams.allow_external_seed }
            : {}),
          ...(secondaryQueryParams?.allow_stale_cache !== undefined
            ? { allow_stale_cache: secondaryQueryParams.allow_stale_cache }
            : {}),
          ...(secondaryQueryParams?.fast_mode !== undefined
            ? { fast_mode: secondaryQueryParams.fast_mode }
            : {}),
        ...
        return {
          ...
          timeout: Math.min(2400, Number(axiosConfig.timeout || 2400)),
          validateStatus: () => true,
          data: buildSearchProductsV2Body({
            payload,
            search: secondarySearch,
            metadata,
            clientChannel,
            gatewayRequestId,
            defaultSearchAllMerchants: secondaryQueryParams?.search_all_merchants === true,
          }),
        };
      }
      return null;
    ...
    try {
      ...
      if (!response && (operation === 'find_products' || operation === 'find_products_multi')) {
        ...
        if (
          ...
          const legacyBeautyMainlineReason =
            err?.code === 'ECONNABORTED'
          ...
          response = {
            ...
            data: withStrictEmptyFallback({
              body: null,
              queryParams,
              reason: legacyBeautyMainlineReason,
              upstreamStatus,
              upstreamCode: upstreamCode || err?.code || null,
              upstreamMessage: upstreamMessage || err?.message || null,
      ...
      if (
        ...
        try {
          ...
          if (expandedSecondaryQuery && expandedSecondaryQuery !== queryText) {
            const secondStageLimit = Math.min(Math.max(requestedLimit * 2, 20), 80);
            const secondStageAxiosConfig = buildFindProductsMultiSecondStageAxiosConfig({
              expandedQuery: expandedSecondaryQuery,
              limit: secondStageLimit,
            });
            if (!secondStageAxiosConfig) {
              secondarySupplementMeta = {
                attempted: true,
                applied: false,
                added_count: 0,
                ...
                reason: 'second_stage_contract_unsupported',
              };
              addFpmGateTrace({
    ...
    if (operation === 'find_products' || operation === 'find_products_multi') {
      ...
      const isStrictEmpty =
        ...
        products.length === 0 &&
        !hasClarification;
      const existingStrictEmptyReason =
        typeof existingMeta?.strict_empty_reason === 'string' &&
        existingMeta.strict_empty_reason.trim()
          ? existingMeta.strict_empty_reason.trim()
          : null;
      ...
      enriched = withSearchDiagnostics(enriched, {
        ...
        ...(isStrictEmpty
          ? {
              strict_empty: true,
              strict_empty_reason: existingStrictEmptyReason || fallbackReason || 'no_candidates',
            }
          : {}),
      });
