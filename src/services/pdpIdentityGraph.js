...
function normalizeExternalProductIdList(value) {
...
function shouldSuppressColorAxisValue(product, value) {
  ...
  if (/^(?:default|default title|title|option|standard default|single item|one piece|single piece|1\s*pieces?|1\s*pcs?)$/i.test(normalized)) return true;
...
function inferAxisFromGenericOptionValue(value) {
  ...
  if (/^(?:default|default title|title|option|standard default|single item|one piece|single piece|1\s*pieces?|1\s*pcs?)$/i.test(normalized)) return null;
...
function parseNamedAxisFromOptions(product, names) {
  ...
  for (const option of optionEntries) {
    ...
    if (/^(?:default|default title|title|option|standard default|single item|one piece|single piece|1\s*pieces?|1\s*pcs?)$/i.test(value)) continue;
...
async function fetchBackfillProducts({
  limit = 500,
  brandFilter = null,
  externalProductIds = [],
  queryFn = query,
  ...
  const normalizedLimit = Math.max(1, Math.min(5000, Number(limit) || 500));
  const exactExternalProductIds = normalizeExternalProductIdList(externalProductIds);
  const sourceLimit = exactExternalProductIds.length
    ? Math.max(normalizedLimit, exactExternalProductIds.length)
    : normalizedLimit;
  ...
  if (!exactExternalProductIds.length) {
    ...
    internalParams.push(sourceLimit);
  ...
  externalParams.push(sourceLimit);
