...
async function fetchCatalogServingEligibleSourceSet(queryFn) {
  // Gate eligibility on catalog_row_trust.serving_decision='public', the single
  // source of truth. The trust gate composes identity + source lifecycle + IPS
  // + quarantine + tombstone + the c1.v0.3 first-party carve-out + c1.v0.4
  // no-IPS-external_seed block in one place — feeding through to is_public
  // (catalogServingIndex.js:709) and on to publish_state on the resulting
  // serving docs, which readers #4 and #5 already consume directly.
  const sql = `SELECT cp.merchant_id, cp.source_product_id
