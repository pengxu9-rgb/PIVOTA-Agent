...
const INCI_STRUCTURE_RE =
  /\b(aqua|water|extract|oil|acid|glycol|glycerin|alcohol|amide|amine|ceramide|panthenol|niacinamide|tocopherol|tocoherol|hyaluronate|triglyceride|dimethicone|siloxane|ferment|butter|wax|lanolin|cera\s+alba|beeswax|bees\s+wax|vitellaria|shea|retinoids?|vitamins?|fragrance|neroli|seed|leaf|root|flower|fruit|sodium|potassium|chloride|oxide|dioxide|hydroxide|carbomer|cellulose|poly|polyisobutene|copolymer|pectin|capry|propanediol|butylene|ethylene|hexanediol|ethanol|glucoside|benzoate|sorbate|retinal|retinol|caffeine|bisabolol|arginine|ergothioneine|isododecane|isostearate|stearate|cinnamate|salicylate|citronellal|ketones?|parfum|limonene|linalool|geraniol|citral|citronellol|eugenol|farnesol|mica|silica|talc|fluorphlogopite|methyl\s+esters?|glutamate|adipate|oleate|palmitate|linoleate|trideceth)\b|peptide/i;
...
function sanitizeIngredientRawText(rawText, { activeOnly = false } = {}) {
  ...
  text = text
    .replace(/^[\s:;,.|-]+/, '')
    .replace(/^\s*(?:shade|shades?)\s*:\s*[^,;|]{1,80}\s+(?=[A-Z][A-Za-z0-9/()' -]+,)/i, '')
    .replace(/^\s*[A-Z0-9][A-Z0-9\s&'+/-]{2,60}\s*:\s*(?=[A-Z][A-Za-z0-9/()' -]+(?:,|\s))/g, '')
    .replace(/\s+/g, ' ')
    .trim();
...
function buildStructuredPdpIngredientModules(product, options = {}) {
  ...
  const forceFillBlocked = isForceFillIngredientDisplayBlocked(product);
