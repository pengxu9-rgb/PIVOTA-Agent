...
const { pdpRouteResolvable } = require('./pdpRenderability');
...
/**
 * Is this row seed-ROUTED? Reuses pdpRenderability's own lane test by pinning seedRouteOk true, so the two
 * files cannot drift on what "seed-routed" means. Valid only while pdpRouteResolvable's non-seed arms all
 * return false — pinned by tests/public_read_chain_seed_routed_probe.node.test.cjs.
 */
