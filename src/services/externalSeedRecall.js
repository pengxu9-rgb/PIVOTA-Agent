...
const RECALL_LEAF_CATEGORY_PATTERNS = Object.freeze([
  ...
  ['Hair Spray', /\b(?:hair\s*spray|hairspray)\b/i],
  ['Hair Gel', /\b(?:hair(?:[\s-]+[a-z0-9+-]+){0,3}[\s-]+gel|edge\s+control\s+gel|styling\s+gel|edge\s+styling\s+set)\b/i],
  ['Dry Shampoo', /\bdry\s+shampoo\b/i],
  ['Hair Milk', /\bhair(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+milks?\b/i],
  ['Leave-In Conditioner', /\b(?:leave[-\s]?in\b(?=.{0,80}\b(?:conditioner|hair|detangling)\b)|detangling\s+leave[-\s]?in)\b/i],
  ['Deep Conditioner', /\b(?:deep(?:[\s-]+[a-z0-9+-]+){0,3}\s+conditioner|moisture\s+repair\s+deep\s+conditioner)\b/i],
  ['Hair Oil', /\bhair(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+oils?\b/i],
  ['Body Oil', /\bbody(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+oils?\b/i],
  ['Makeup Remover', /\b(?:make[-\s]?up(?:[\s-]+[a-z0-9+-]+){0,4}(?:remover|removing|melting)|micellar)\b/i],
  ['Lip Balm', /\blip(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+balms?\b/i],
  ['Lip Oil', /\blip(?:[\s-]+[a-z0-9+-]+){0,3}[\s-]+oils?\b/i],
  ['Hair Mask', /\bhair(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+masks?\b/i],
  ['Hair Set', /\b(?:maintenance crew|repair bundle|hair(?:\s+care)?\s+(?:duo|set|sets|kit|kits|bundle|bundles|crew))\b/i],
  ['Hair Tool', /\b(?:detangling\s+brush|hair\s+brush|hair\s+comb)\b/i],
  ['Face Mist', /\bface(?:[\s-]+[a-z0-9+-]+){0,3}[\s-]+mists?\b/i],
  ['Face Oil', /\b(?:face|facial|hydrating|recovery|barrier|balancing|glow)(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+oils?\b/i],
  ['Makeup Brush', /\b(?:makeup\s+brush|setting\s+brush|powder\s+brush|foundation\s+brush|brush\s+\d{2,})\b/i],
  ['Makeup Sponge', /\b(?:makeup\s+sponge|blending\s+sponge|beauty\s+sponge|powder\s+puff|precision\s+makeup\s+sponge|mushroom\s+sponge|sponge)\b/i],
  ['Setting Spray', /\b(?:setting\s+spray|fixing\s+mist|makeup[-\s]+fixing\s+mist|makeup[-\s]+extending\s+setting\s+spray)\b/i],
  ['Brow Pencil', /\b(?:brow\s+pencil|eyebrow\s+pencil|ultra\s+fine\s+brow\s+pencil|brow\s+mvp|brow\s+definer|brow\s+styler)\b/i],
  ['Brow Gel', /\b(?:brow\s+gel|eyebrow\s+gel|brow\s+harmony|kybrow\s+gel|tint\s+\+\s+tame\s+brow)\b/i],
  ['Deodorant', /\b(?:deodorant|fresh[-\s]*feel\s+deodorant)\b/i],
  ['Hand Cream', /\bhand(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+creams?\b/i],
  ['Body Cream', /\bbody(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+creams?\b/i],
  ...
  ['Blush', /\bblush(?:es)?\b/i],
  ['Bronzer', /\b(?:bronzers?|contour|cream\s+bronzer|freestyle\s+cream\s+bronzer)\b/i],
  ['Lipstick', /\blipsticks?\b/i],
  ['Lip Gloss', /\b(?:lip\s+gloss(?:es)?|gloss\s+bomb|lip\s+luminiz(?:er|ers?)|lip\s+plump(?:er|ers?)?)\b/i],
  ['Highlighter', /\b(?:highlighters?|illuminator|luminizer|luminiser|killawatt)\b/i],
  ...
  ['Clay Mask', /\b(?:clay(?:[\s-]+[a-z0-9+-]+){0,4}[\s-]+mask|clay\s+stick\s+mask)\b/i],
