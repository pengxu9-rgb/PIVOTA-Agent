...
function resolveKey(ctx, key) {
  ...
  if (key.startsWith('userProfile.')) return getByPath(ctx.userProfile, key.slice('userProfile.'.length));
  if (key.startsWith('userSignals.')) return getByPath(ctx.userSignals, key.slice('userSignals.'.length));
